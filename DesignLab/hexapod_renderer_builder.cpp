...
HexapodRendererBuilder::Build(
  ...
  if (display_quality == DisplayQuality::kHigh) {
    const auto renderer = std::make_shared<PhantomXMkIIRendererModel>(
        converter_ptr, calculator_ptr);

    return {renderer, renderer};
  } else {
    const auto renderer = std::make_shared<PhantomXRendererSimple>(
        converter_ptr, calculator_ptr, display_quality);
