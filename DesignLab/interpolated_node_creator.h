...
class InterpolatedNodeCreator final {
 public:
  ...
  //! @return 補間されたノード.
  std::vector<RobotStateNode> CreateInterpolatedNode(
      const RobotStateNode& node, const RobotStateNode& next_node) const;

 private:
  static constexpr float kInterpolatedDistance = 0.15f;  //!< 補間する距離 [mm].
  static constexpr float kInterpolatedAngle =
      math_util::ConvertDegToRad(0.1f);  //!< 補間する角度 [rad].

  static constexpr float kBodyMoveMaxInterpolatedDistance =
      200.0f;  //!< これ以上の距離を移動する場合は,補間を行わない.

  //! @brief 補間が必要ないかどうかを判定する.
  //! 重心座標の変化と,脚の接地点の変化,ロボットの姿勢の変化を調べて,
  //! 変化がないならば,補間が必要ないと判定する.
  //! @return 補間が必要ないならば trueを返す.
  bool IsNoChange(const RobotStateNode& current_node,
                  const RobotStateNode& next_node) const;

  //! @return 胴体が移動するなら trueを返す.
  bool IsBodyMove(const RobotStateNode& current_node,
                  const RobotStateNode& next_node) const;

  //! @return 胴体が回転するなら trueを返す.
  bool IsBodyRot(const RobotStateNode& current_node,
                 const RobotStateNode& next_node) const;
