...
namespace dl_leg
{
	...
	std::bitset<LEG_STATE_BIT_NUM> makeLegState(ComType::EComPattern com_pattern, const bool is_ground[HexapodConst::LEG_NUM], const EDiscreteLegPos discretized_leg_pos[HexapodConst::LEG_NUM]);
	...
	//! @param [in] leg_state 現在の脚状態
	//! @return ComType::EComPattern 重心パターンを返す．
	ComType::EComPattern getComPatternState(const std::bitset<LEG_STATE_BIT_NUM>& leg_state);
	...
	std::bitset<LEG_STATE_BIT_NUM> changeComPattern(const std::bitset<LEG_STATE_BIT_NUM>& leg_state, ComType::EComPattern new_com_pattern);
