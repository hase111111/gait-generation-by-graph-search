...
class CameraDragger final : public IDxlibDraggable, public IDxlibWheelHandler {
 public:
  //! @param[in] camera_manager
  //! カメラの状態を管理するクラスをポインタで受け取る.
  explicit CameraDragger(const std::shared_ptr<DxlibCamera> camera);

  bool IsDraggable([[maybe_unused]] int cursor_x,
  ...
  void DraggedAction(int cursor_dif_x, int cursor_dif_y,
                     unsigned int mouse_key_bit) override;

  bool CanHandleWheel([[maybe_unused]] int cursor_x,
                      [[maybe_unused]] int cursor_y) const override {
    // カメラの操作はどこをクリックしても可能.
    return true;
  };
