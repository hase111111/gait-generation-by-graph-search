...
public:
	...
	//! @return dl_vec::SVector ローカル座標の脚座標
	dl_vec::SVector convertLocalLegPos(const SNode& node, const dl_vec::SVector& global_pos, const int leg_num, const bool do_consider_rot) const;

	//! @brief 脚座標は脚の付け根を原点とした座標系なので，それをグローバル座標に変換する．
	//! @param [in] _node ロボットの状態を表すノード
	//! @param [in] leg_num 脚番号 0～5
	//! @param [in] _consider_rot 回転を考慮するかどうか
	//! @return dl_vec::SVector グローバル座標の脚座標
	inline dl_vec::SVector getGlobalLegPos(const SNode& _node, const int _leg_num, const bool _consider_rot) const
	{
	...
	//! @param [in] leg_num 脚番号 0～5
	//! @param [in] _consider_rot 回転を考慮するかどうか
	//! @return dl_vec::SVector グローバル座標の脚の基準地点の座標
	inline dl_vec::SVector getGlobalLegBasePos(const SNode& _node, const int _leg_num, const bool _consider_rot) const
	{
	...
	// @param [in] leg_num 脚番号 0～5
	// @param [in] _consider_rot 回転を考慮するかどうか
	// @return dl_vec::SVector グローバル座標の脚の付け根の座標
	inline dl_vec::SVector getGlobalCoxaJointPos(const SNode& _node, const int _leg_num, const bool _consider_rot) const
	{
	...
	dl_vec::SVector getGlobalFemurJointPos(const SNode& _node, const int _leg_num) const;

	//【calclateJointPos関数を使用してから使うこと!!】tibia joint (第3関節) の座標を返す．回転を考慮したグローバル座標.
	dl_vec::SVector getGlobalTibiaJointPos(const SNode& _node, const int _leg_num) const;
	...
	//! @param leg_num 脚番号
	//! @return 可動範囲内ならtrueを返す．
	bool isLegInRange(const dl_vec::SVector& local_leg_pos, const int leg_num) const;
...
private:
	...
	dl_vec::SVector getLocalCoxaJointPos(const int leg_num) const
	{
		if (leg_num == 0) { return dl_vec::SVector(HexapodConst::BODY_FRONT_LENGTH, -HexapodConst::BODY_FRONT_WIDTH, 0.0f); }	// 脚0 右上
		else if (leg_num == 1) { return dl_vec::SVector(0.0f, -HexapodConst::BODY_CENTER_WIDTH, 0.0f); }	// 脚1 右横
		else if (leg_num == 2) { return dl_vec::SVector(-HexapodConst::BODY_REAR_LENGTH, -HexapodConst::BODY_REAR_WIDTH, 0.0f); }	// 脚2 右下
		else if (leg_num == 3) { return dl_vec::SVector(-HexapodConst::BODY_REAR_LENGTH, HexapodConst::BODY_REAR_WIDTH, 0.0f); }	// 脚3 左下
		else if (leg_num == 4) { return dl_vec::SVector(0.0f, HexapodConst::BODY_CENTER_WIDTH, 0.0f); }	// 脚4 左横
		else if (leg_num == 5) { return dl_vec::SVector(HexapodConst::BODY_FRONT_LENGTH, HexapodConst::BODY_FRONT_WIDTH, 0.0f); }	// 脚5 左上

		return dl_vec::SVector(0, 0, 0);
	}
	...
	dl_vec::SVector m_local_femurjoint_pos[HexapodConst::LEG_NUM];	//FemurJoint(第2関節)の位置．脚の付け根を原点とするローカル座標．calclateJointPos関数で値をセットする．
	dl_vec::SVector m_local_tibiajoint_pos[HexapodConst::LEG_NUM];	//TibiaJoint(第3関節)の位置．脚の付け根を原点とするローカル座標．calclateJointPos関数で値をセットする．
