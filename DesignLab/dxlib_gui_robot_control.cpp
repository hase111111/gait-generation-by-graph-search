...
//! @file dxlib_gui_robot_control.cpp
...
DxlibGuiRobotControl::DxlibGuiRobotControl(
  ...
  button_.push_back(std::make_unique<SimpleButton>("Update", left_pos_x,
                                                   top_pos_y + button_range,
  ...
  // シリアル通信を開始する.
  serial_communication_thread_ptr_ = std::make_unique<boost::thread>(
...
void DxlibGuiRobotControl::DrawString() const {
  ...
  if (serial_communication_.IsEnd()) {
    DrawFormatStringToHandle(
        ...
        str_color, font_handle_, "スレッドは終了しています.");
  } else {
    DrawFormatStringToHandle(gui_left_pos_x_ + 10,
                             text_top_y + text_interval_y * (text_line++),
                             str_color, font_handle_, "スレッドは実行中です.");
  }

  // 通信されたデータの数を出力する.
  const std::vector<std::string> read_data =
      serial_communication_.GetAllReadData();
  DrawFormatStringToHandle(
      gui_left_pos_x_ + 10, text_top_y + text_interval_y * (text_line++),
      str_color, font_handle_, "受信したデータの数: %d", read_data.size());
  ...
  for (auto i = read_data.rbegin(); i != read_data.rend(); ++i) {
    std::string str = "[" +
                      std::to_string(static_cast<int>(read_data.size()) -
                                     (10 - display_num)) +
                      "] ";
    str += *i;
    ...
    DrawFormatStringToHandle(gui_left_pos_x_ + 10,
                             text_top_y + text_interval_y * (text_line++),
                             str_color, font_handle_, "%s", str.c_str());
