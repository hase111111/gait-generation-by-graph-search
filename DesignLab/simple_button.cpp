...
void SimpleButton::SetPos(const int pos_x, const int pos_y,
...
void SimpleButton::Draw() const {
  ...
  const int button_color =
      GetColor(255 - now_color_blue_, 255 - now_color_blue_ / 2, 255);
  const int str_color = GetColor(20, 20, 20);
  const int frame_size = 1;

  // ベースを描画.
  DrawBox(pos_middle_x - kSizeX / 2, pos_middle_y - kSizeY / 2,
          pos_middle_x + kSizeX / 2, pos_middle_y + kSizeY / 2, base_color,
          TRUE);

  // その上にボタンを描画.
  DrawBox(pos_middle_x - kSizeX / 2 + frame_size,
          pos_middle_y - kSizeY / 2 + frame_size,
          pos_middle_x + kSizeX / 2 - frame_size,
          pos_middle_y + kSizeY / 2 - frame_size, button_color, TRUE);
  ...
  for (int i = 0; i < text_.size(); ++i) {
    const auto text_width = GetDrawStringWidthToHandle(
        text_[i].c_str(), static_cast<int>(text_[i].size()), font_handle);

    const auto text_height = static_cast<int>(text_.size()) * kFontSize;

    DrawStringToHandle(pos_middle_x - text_width / 2,
                       pos_middle_y - text_height / 2 + i * kFontSize,
                       text_[i].c_str(), str_color, font_handle);
  }
}
...
void SimpleButton::ClickedAction(const DxlibMouseState& state) {
  ...
  if (click_function_ && visible_ && state.left_pushing_count == 1) {
    now_color_blue_ = 128;
...
bool SimpleButton::CursorOnGui(const int cursor_x,
                               const int cursor_y) const noexcept {
  return pos_middle_x - kSizeX / 2 < cursor_x &&
         cursor_x < pos_middle_x + kSizeX / 2 &&
         pos_middle_y - kSizeY / 2 < cursor_y &&
         cursor_y < pos_middle_y + kSizeY / 2;
}
...
int SimpleButton::GetFitButtonSizeX(const int now_size_x) const noexcept {
  ...
  for (const auto& i : text_) {
    const int width = GetDrawStringWidthToHandle(
        i.c_str(), static_cast<int>(i.size()), font_handle);
