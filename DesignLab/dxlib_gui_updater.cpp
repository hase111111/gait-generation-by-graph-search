...
void DxlibGuiUpdater::RegisterGui(const std::shared_ptr<IDxlibGui> gui_ptr,
                                  int priority) {
...
void DxlibGuiUpdater::ActivateClickable(
  ...
  for (auto i = clickable_ptrs_.rbegin(); i != clickable_ptrs_.rend(); ++i) {
    if ((*i).second->CursorOnGui(mouse_ptr->GetCursorPosX(),
                                 mouse_ptr->GetCursorPosY())) {
      (*i).second->ClickedAction(
          {.cursor_x = mouse_ptr->GetCursorPosX(),
           .cursor_y = mouse_ptr->GetCursorPosY(),
           .left_pushing_count = mouse_ptr->GetPressingCount(MOUSE_INPUT_LEFT),
...
void DxlibGuiUpdater::ActivateDraggable(
    const std::shared_ptr<const Mouse> mouse_ptr) {
  ...
  if (pressing_count == 0) {
    ...
    for (auto i = draggable_ptrs_.rbegin(); i != draggable_ptrs_.rend(); ++i) {
      if ((*i).second->IsDraggable(mouse_ptr->GetCursorPosX(),
                                   mouse_ptr->GetCursorPosY())) {
...
void DxlibGuiUpdater::ActivateWheelHandler(
  ...
  for (auto i = wheel_handler_ptrs_.rbegin(); i != wheel_handler_ptrs_.rend();
       ++i) {
    if ((*i).second->CanHandleWheel(mouse_ptr->GetCursorPosX(),
                                    mouse_ptr->GetCursorPosY())) {
