...
ComCandidatePolygonMaker::ComCandidatePolygonMaker(const std::shared_ptr<const IHexapodCoordinateConverter>& converter_ptr) :
...
void ComCandidatePolygonMaker::MakeCandidatePolygon(const RobotStateNode& node, std::array<ComPosAndPolygon, MAKE_POLYGON_NUM>* output_poly) const
{
	...
	std::array<Vector2, HexapodConst::kLegNum> leg_pos_xy;	//_XY平面に射影した脚位置を算出する．(グローバル座標系)
...
void ComCandidatePolygonMaker::MakeCandidateBox(const std::array<Vector2, HexapodConst::kLegNum>& leg_pos, const int start_leg_num, Polygon2* output_poly) const
{
	//脚位置を線で結ぶ．この交点から重心候補地点が存在する多角形を求める
	LineSegment2 leg_line_02(leg_pos[(start_leg_num + 0) % HexapodConst::kLegNum], leg_pos[(start_leg_num + 2) % HexapodConst::kLegNum]);
	LineSegment2 leg_line_03(leg_pos[(start_leg_num + 0) % HexapodConst::kLegNum], leg_pos[(start_leg_num + 3) % HexapodConst::kLegNum]);
	LineSegment2 leg_line_14(leg_pos[(start_leg_num + 1) % HexapodConst::kLegNum], leg_pos[(start_leg_num + 4) % HexapodConst::kLegNum]);
	LineSegment2 leg_line_15(leg_pos[(start_leg_num + 1) % HexapodConst::kLegNum], leg_pos[(start_leg_num + 5) % HexapodConst::kLegNum]);
	LineSegment2 leg_line_25(leg_pos[(start_leg_num + 2) % HexapodConst::kLegNum], leg_pos[(start_leg_num + 5) % HexapodConst::kLegNum]);
	...
	//中心と0番の脚位置を結んだ線分を求める
	LineSegment2 leg_line_0_center(leg_pos[(start_leg_num + 0) % HexapodConst::kLegNum], intersection_03_14);
...
void ComCandidatePolygonMaker::MakeCandidateTriangle(const std::array<Vector2, HexapodConst::kLegNum>& leg_pos, ComPosAndPolygon* output) const
{
