...
public:
	...
	std::optional<T> Import(const std::string& file_path) const
	{
		...
		{
			if (do_output_message_) { ::designlab::cmdio::Output("データのシリアライズに失敗しました．", OutputDetail::kSystem); }
		...
		if (do_output_message_) { ::designlab::cmdio::Output("データの検証に成功しました．", OutputDetail::kSystem); }
