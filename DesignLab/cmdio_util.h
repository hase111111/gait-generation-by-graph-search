...
//! @n 設定ファイルで出力の許可をしていない場合は，出力しない機能を実装するため作成した．
namespace designlab::cmdio
{
...
void SetOutputLimit(::designlab::enums::OutputDetail limit);
...
void Output(const std::string& str, ::designlab::enums::OutputDetail detail);

//! @brief 中央に文字を出力する関数．文字列が長すぎる場合は普通に出力される．
//! @param [in] str 出力する文字列
//! @param [in] detail 出力する文字列の詳細
void OutputCenter(const std::string& str, ::designlab::enums::OutputDetail detail);

//! @brief 右端に文字を出力する関数．文字列が長すぎる場合は普通に出力される．
//! @param [in] str 出力する文字列
//! @param [in] detail 出力する文字列の詳細
void OutputRight(const std::string& str, ::designlab::enums::OutputDetail detail);


//! @brief コマンドラインで改行をする関数．
//! @param [in] num 改行する回数．0以下の値を入れると何もしない．
//! @param [in] detail 出力する文字列の詳細
void OutputNewLine(int num, ::designlab::enums::OutputDetail detail);

//! @brief コマンドラインに水平線を出力する関数．
//! @param [in] line_visual 水平線の見た目．'-'ならば水平線，'='ならば二重水平線
//! @n 2文字以上の文字列を入れると動作しない．
//! @param [in] detail 出力する文字列の詳細
void OutputHorizontalLine(const std::string& line_visual, ::designlab::enums::OutputDetail detail);
