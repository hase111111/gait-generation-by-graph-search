...
//! @file stability_margin_renderer.cpp
...
void StabilityMarginRenderer::Draw() const {
  ...
  Vector3 polygon_sum{0, 0,
                      0};  // 多角形の頂点の合計,重心を求めるために使用する

  for (int i = 0; i < HexapodConst::kLegNum; ++i) {
    if (leg_func::IsGrounded(node_.leg_state, i)) {
      polygon.push_back(converter_ptr_->ConvertLegToGlobalCoordinate(
          node_.leg_pos[i], i, node_.center_of_mass_global_coord, node_.posture,
          true));

      polygon.back() +=
          Vector3{0, 0, 5};  // わかりやすさのため,高さを少し上げる.
  ...
  VECTOR projected_center_pos = dxlib_util::ConvertToDxlibVec(
      ...
      {node_.center_of_mass_global_coord.x, node_.center_of_mass_global_coord.y,
       center.z + 10});
