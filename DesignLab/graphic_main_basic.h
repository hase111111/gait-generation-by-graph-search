...
class GraphicMainBasic final : public IGraphicMain {
 ...
 private:
  ...
  const int
      kInterpolatedAnimeCount;  //!< 補間されたノードの表示を切り替える間隔.

  const std::shared_ptr<const GraphicDataBroker>
      broker_ptr_;  //!< データを受け取るクラス.
  ...
  InterpolatedNodeCreator
      interpolated_node_creator_;  //!< ノード間を補間するクラス.
  ...
  std::vector<RobotStateNode>
      interpolated_node_;  //!< 補間されたノードを記録する配列.

  int interpolated_anime_start_count_{
      0};  //!< 補間されたノードを表示し始めるカウント.
