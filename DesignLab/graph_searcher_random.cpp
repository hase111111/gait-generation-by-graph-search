...
std::tuple<GraphSearchResult, int, int> GraphSearcherRandom::SearchGraphTree(
  ...
  [[maybe_unused]] const DividedMapState& devide_map_state,
...
) const
{
    ...
    if (graph.IsEmpty())
    {
        ...
        { enums::Result::kFailure, "グラフが空のため評価できません．" };
    ...
    if (depth1_node.empty())
    {
        // 深さ1のノードが存在しないなら，終了．
        const GraphSearchResult result =
        { enums::Result::kFailure, "深さ1のノードが存在しません．" };

        return { result, -1, -1 };
    }

    // ランダムなやつを一つ選択する．
    const int select_index = (depth1_node.size() == 1) ?
        0 : math_util::GenerateRandomNumber(0, static_cast<int>(depth1_node.size()) - 1);
