...
class NodeCreatorBodyRot final : public INodeCreator {
 ...
 private:
  ...
  //! ロボットのヨー軸周りの回転角度.
  static constexpr float kBodyYawRotAngleMin =
      math_util::ConvertDegToRad(-20.f);

  //! ロボットのヨー軸周りの回転角度の分割数（奇数にすること）.
  static constexpr int kBodyYawRotAngleDivNum = 21;

  constexpr std::array<float, kBodyYawRotAngleDivNum> CreateCandidateAngle()
      const {
  ...
  const std::array<float, kBodyYawRotAngleDivNum> candidate_angle_ =
      CreateCandidateAngle();
  ...
  static_assert(
      kBodyYawRotAngleMax > kBodyYawRotAngleMin,
