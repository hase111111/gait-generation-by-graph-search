...
//! @details
//! カメラを,上から見るのか,横から見るのか,切り替えるのは Dxlib
//! の場合操作が少々難しい.
...
class DxlibCamera final {
 ...
 private:
  static constexpr float kDefaultCameraZoom{
      1500.f};  //!< カメラのズームの初期値.
  ...
  constexpr static bool kOutputDebugLog =
      false;  //!< デバッグログを出力するかどうか.

  const Vector3
      kDefaultCameraFrontVec;  //!< デフォルトのカメラの方向を表す単位ベクトル.

  const Vector3
      kDefaultCameraUpVec;  //!< デフォルトのカメラの上方向を表す単位ベクトル.

  enums::CameraViewMode camera_view_mode_;  //!< カメラの視点を決定する.

  Vector3
      free_controlled_target_pos_;  //!< カメラの注視点を自由に操作する際の注視点の座標.
