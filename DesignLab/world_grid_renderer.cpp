...
void WorldGridRenderer::Draw() const {
  ...
  for (int i = 0; i < kMainGridNum + 1 + kMainGridNum; i++) {
    ...
    for (int j = 0; j < kSubGridDividedNum - 1; j++) {
      ...
      start_pos = VGet(kGridMinX,
                       kGridMinY + kMainGridInterval * i +
                           kMainGridInterval / kSubGridDividedNum * (j + 1),
                       kGridLineZPos);
      end_pos = VGet(kGridMaxX,
                     kGridMinY + kMainGridInterval * i +
                         kMainGridInterval / kSubGridDividedNum * (j + 1),
                     kGridLineZPos);
      DrawLine3D(start_pos, end_pos, kSubGridXColor);

      start_pos = VGet(kGridMinX + kMainGridInterval * i +
                           kMainGridInterval / kSubGridDividedNum * (j + 1),
                       kGridMinY, kGridLineZPos);
      end_pos = VGet(kGridMinX + kMainGridInterval * i +
                         kMainGridInterval / kSubGridDividedNum * (j + 1),
                     kGridMaxY, kGridLineZPos);
      DrawLine3D(start_pos, end_pos, kSubGridYColor);
    }

    SetDrawBlendMode(DX_BLENDMODE_NOBLEND,
                     0);  // 半透明を解除する.これを忘れると描画がおかしくなる.
  }
}
