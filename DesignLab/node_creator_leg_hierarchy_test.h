...
TEST_SUITE("NodeCreatorLegHierarchyTest") {
  ...
  TEST_CASE("CreateTest_WhenOneLegIsSwinging") {
    ...
    NodeCreatorLegHierarchy node_creator_leg_hierarchy{move, discrete_leg_pos};

    // 1脚のみ遊脚させる.
    std::array<bool, HexapodConst::kLegNum> ground_leg = {false, true, true,
                                                          true,  true, true};

    RobotStateNode robot_state_node;

    robot_state_node.leg_state =
        MakeLegStateBit(DiscreteComPos::kCenterFront, ground_leg,
