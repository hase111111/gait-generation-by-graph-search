...
PhantomXMkII::PhantomXMkII(const PhantomXMkIIParameterRecord& parameter_record)
    : kBodyLiftingHeightMin(parameter_record.body_lifting_height_min),
      ...
      kMovableCoxaAngleMin(math_util::ConvertDegToRad(
          parameter_record.movable_coxa_angle_min_deg)),
      kMovableCoxaAngleMax(math_util::ConvertDegToRad(
          parameter_record.movable_coxa_angle_max_deg)),
      ...
      free_leg_pos_leg_coordinate_({{
          {160 * cos(PhantomXMkIIConst::kCoxaDefaultAngle[0]),
           160 * sin(PhantomXMkIIConst::kCoxaDefaultAngle[0]), kFreeLegHeight},
          {160 * cos(PhantomXMkIIConst::kCoxaDefaultAngle[1]),
           160 * sin(PhantomXMkIIConst::kCoxaDefaultAngle[1]), kFreeLegHeight},
          {160 * cos(PhantomXMkIIConst::kCoxaDefaultAngle[2]),
           160 * sin(PhantomXMkIIConst::kCoxaDefaultAngle[2]), kFreeLegHeight},
          {160 * cos(PhantomXMkIIConst::kCoxaDefaultAngle[3]),
           160 * sin(PhantomXMkIIConst::kCoxaDefaultAngle[3]), kFreeLegHeight},
          {160 * cos(PhantomXMkIIConst::kCoxaDefaultAngle[4]),
           160 * sin(PhantomXMkIIConst::kCoxaDefaultAngle[4]), kFreeLegHeight},
          {160 * cos(PhantomXMkIIConst::kCoxaDefaultAngle[5]),
           160 * sin(PhantomXMkIIConst::kCoxaDefaultAngle[5]), kFreeLegHeight},
      }}),
      leg_base_pos_robot_coordinate_({{
          {PhantomXMkIIConst::kCoxaBaseOffsetX,
           -PhantomXMkIIConst::kCoxaBaseOffsetY,
           PhantomXMkIIConst::kCoxaBaseOffsetZ},  // 脚0 右上.
          {0.0f, -PhantomXMkIIConst::kCenterCoxaBaseOffsetY,
           PhantomXMkIIConst::kCoxaBaseOffsetZ},  // 脚1 右横.
          {-PhantomXMkIIConst::kCoxaBaseOffsetX,
           -PhantomXMkIIConst::kCoxaBaseOffsetY,
           PhantomXMkIIConst::kCoxaBaseOffsetZ},  // 脚2 右下.
          {-PhantomXMkIIConst::kCoxaBaseOffsetX,
           PhantomXMkIIConst::kCoxaBaseOffsetY,
           PhantomXMkIIConst::kCoxaBaseOffsetZ},  // 脚3 左下.
          {0.0f, PhantomXMkIIConst::kCenterCoxaBaseOffsetY,
           PhantomXMkIIConst::kCoxaBaseOffsetZ},  // 脚4 左横.
          {PhantomXMkIIConst::kCoxaBaseOffsetX,
           PhantomXMkIIConst::kCoxaBaseOffsetY,
           PhantomXMkIIConst::kCoxaBaseOffsetZ},  // 脚5 左上.
      }}),
...
HexapodJointState PhantomXMkII::CalculateJointState(
  ...
  // femur jointの計算.
  {
    const Vector3 femur_joint_pos = Vector3{
        ...
        PhantomXMkIIConst::kCoxaLength * std::sin(res.joint_angle[0]), 0};
    ...
    if (!math_util::CanMakeTriangle((leg_pos - femur_joint_pos).GetLength(),
      ...
      angle_ft_phase = (angle_ft_phase > std::numbers::pi_v<float>)
                           ? angle_ft_phase - std::numbers::pi_v<float>
                           : angle_ft_phase;

      const Vector3 candidate_leg_pos =
          femur_joint_pos +
          Vector3{(PhantomXMkIIConst::kFemurLength +
                   PhantomXMkIIConst::kTibiaLength) *
      ...
      const Vector3 candidate_leg_pos_phase =
          femur_joint_pos +
          Vector3{(PhantomXMkIIConst::kFemurLength * std::cos(angle_ft_phase) +
      ...
      const float distance_phase =
          (leg_pos - candidate_leg_pos_phase).GetLength();
      ...
      res.joint_pos_leg_coordinate[2] =
          femur_joint_pos +
          Vector3{PhantomXMkIIConst::kFemurLength * std::cos(angle_f) *
                      std::cos(res.joint_angle[0]),
      ...
      res.joint_pos_leg_coordinate[3] =
          res.joint_pos_leg_coordinate[2] +
          Vector3{
  ...
  const float femur_to_leg_x =
      ...
      ((leg_pos.ProjectedXY().GetSquaredLength() >
        res.joint_pos_leg_coordinate[1].ProjectedXY().GetSquaredLength())
           ? 1.f
           : -1.f);
...

  {
    ...
    const float arc_cos_lower =
        2 * PhantomXMkIIConst::kFemurLength * femur_to_leg.GetLength();

    const float arc_cos_arg = arc_cos_upper / arc_cos_lower;

    const float femur_joint_angle =
        std::acos(arc_cos_arg) + std::atan2(femur_to_leg_z, femur_to_leg_x);
  ...
  // tibia jointの計算.
  {
    const Vector3 femur_to_tibia =
        Vector3{PhantomXMkIIConst::kFemurLength * std::cos(res.joint_angle[0]) *
    ...
    const Vector3 tibia_joint_pos =
        res.joint_pos_leg_coordinate[1] + femur_to_tibia;
  ...
  // tibia angleの計算.
  {
    const float tibia_angle =
        std::atan2((femur_to_leg_z - PhantomXMkIIConst::kFemurLength *
                                         std::sin(res.joint_angle[1])),
                   (femur_to_leg_x - PhantomXMkIIConst::kFemurLength *
                                         std::cos(res.joint_angle[1]))) -
        res.joint_angle[1];
...
bool PhantomXMkII::IsValidJointState(
  ...
  if (!PhantomXMkIIConst::IsValidCoxaAngle(leg_index,
                                           joint_state.joint_angle[0])) {
    return false;
  }
  ...
  if (!math_util::IsEqual((joint_state.joint_pos_leg_coordinate[0] -
                           joint_state.joint_pos_leg_coordinate[1])
                              .GetLength(),
  ...
  if (!math_util::IsEqual((joint_state.joint_pos_leg_coordinate[1] -
                           joint_state.joint_pos_leg_coordinate[2])
                              .GetLength(),
                          PhantomXMkIIConst::kFemurLength)) {
    return false;
  }
  ...
  if (!math_util::IsEqual((joint_state.joint_pos_leg_coordinate[2] -
                           joint_state.joint_pos_leg_coordinate[3])
                              .GetLength(),
                          PhantomXMkIIConst::kTibiaLength)) {
    return false;
  }
  ...
  if (joint_state.joint_pos_leg_coordinate[3] != leg_pos) {
    return false;
  }
...
Vector3 PhantomXMkII::ConvertGlobalToLegCoordinate(
    const Vector3& converted_position, const int leg_index,
...
Vector3 PhantomXMkII::ConvertLegToGlobalCoordinate(
  ...
  if (consider_rot) {
    return RotateVector3(
               converted_position + GetLegBasePosRobotCoordinate(leg_index),
               robot_quat) +
...
Vector3 PhantomXMkII::ConvertRobotToGlobalCoordinate(
    const Vector3& converted_position, const Vector3& center_of_mass_global,
    const Quaternion& robot_quat, const bool consider_rot) const {
  if (consider_rot) {
    return RotateVector3(converted_position, robot_quat) +
           center_of_mass_global;
  } else {
    return converted_position + center_of_mass_global;
  }
...
Vector3 PhantomXMkII::ConvertRobotToLegCoordinate(
    const Vector3& converted_position, const int leg_index) const {
  return converted_position - GetLegBasePosRobotCoordinate(leg_index);
}

Vector3 PhantomXMkII::ConvertLegToRobotCoordinate(
    const Vector3& converted_position, const int leg_index) const {
  return converted_position + GetLegBasePosRobotCoordinate(leg_index);
}

Vector3 PhantomXMkII::GetFreeLegPosLegCoordinate(
    const int leg_index) const noexcept {
  // leg_indexは 0～5 である.
  assert(0 <= leg_index);
  assert(leg_index < HexapodConst::kLegNum);
...
Vector3 PhantomXMkII::GetLegBasePosRobotCoordinate(
    const int leg_index) const noexcept {
  // leg_indexは 0～5 である.
  assert(0 <= leg_index);
  assert(leg_index < HexapodConst::kLegNum);
...
bool PhantomXMkII::IsLegInRange(const int leg_index,
                                const Vector3& leg_pos) const {
  // leg_indexは 0～5 である.
  assert(0 <= leg_index);
  assert(leg_index < HexapodConst::kLegNum);
  ...
  if (static_cast<int>(leg_pos.z) < -kMaxLegRSize ||
      0 < static_cast<int>(leg_pos.z)) {
    return false;
  }
...
bool PhantomXMkII::IsLegInterfering(
  ...
  Vector2
      joint_pos_xy[HexapodConst::kLegNum];  // 脚の根元の座標(ロボット座標系).
  ...
  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    ...
    if (line1.HasIntersection(line2)) {
      return true;
    }

    // 脚先の距離を確認する.
    if (leg_pos_xy[i].GetDistanceFrom(
            leg_pos_xy[(i + 1) % HexapodConst::kLegNum]) < kMinLegDistance) {
      return true;
    }
  }
...
float PhantomXMkII::CalculateStabilityMargin(
  ...
  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    if (leg_func::IsGrounded(leg_state, i)) {
      ground_leg_pos[ground_leg_pos_num] =
          leg_pos[i].ProjectedXY() +
  ...
  for (int i = 0; i < ground_leg_pos_num; i++) {
    Vector2 i_to_i_plus_1 =
        ground_leg_pos[(i + 1) % ground_leg_pos_num] - ground_leg_pos[i];
...
bool PhantomXMkII::IsBodyInterferingWithGround(
    const RobotStateNode& node, const DividedMapState& devide_map) const {
  // 重心の干渉を調べる.
  {
  ...
  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    ...
    if (devide_map.IsInMap(leg_pos_global_coord)) {
      const float leg_top_z = (std::max)(
          devide_map.GetTopZ(
              devide_map.GetDividedMapIndexX(leg_pos_global_coord.x),
...
std::array<float, PhantomXMkII::kMaxLegRSize> PhantomXMkII::InitMaxLegR()
    const {
  // 2019年度の先行研究のコードそのまま.
  ...
  // 逆運動学と運動学を行った結果が半径 Permission ^ 0.5
  // の円の中なら等しいと考える.
  ...
  for (int iz = 0; iz < 200; iz++) {
    for (int ix = 53; ix < 248; ix++) {
      // 脚先座標（ローカル）
      const Vector3 line_end(static_cast<float>(ix), 0.0f,
      ...
      // 絶対に正.
      float _im =
          sqrt(math_util::Squared(_IK_trueX) + math_util::Squared(line_end.z));
      ...
      // im = 0 だと定義域エラー.
      const float _q2 =
          acos((math_util::Squared(PhantomXMkIIConst::kFemurLength) +
                math_util::Squared(_im) -
                math_util::Squared(PhantomXMkIIConst::kTibiaLength)) /
               (2.0f * PhantomXMkIIConst::kFemurLength * _im));

      const float _femur_angle = _q1 + _q2;
      const float _tibia_angle =
          acos((math_util::Squared(PhantomXMkIIConst::kFemurLength) +
                math_util::Squared(PhantomXMkIIConst::kTibiaLength) -
                math_util::Squared(_im)) /
               (2.0f * PhantomXMkIIConst::kFemurLength *
                PhantomXMkIIConst::kTibiaLength)) -
          std::numbers::pi_v<float> / 2.0f;

      // 運動学
      const float _K_trueX =
          PhantomXMkIIConst::kFemurLength * cos(_femur_angle) +
          PhantomXMkIIConst::kTibiaLength *
              cos(_femur_angle + _tibia_angle -
                  std::numbers::pi_v<float> / 2.0f);

      Vector3 _kinematics;
      _kinematics.x = _K_trueX + PhantomXMkIIConst::kCoxaLength;
      _kinematics.y = 0.0f;
      _kinematics.z = -(PhantomXMkIIConst::kFemurLength * sin(_femur_angle) +
                        PhantomXMkIIConst::kTibiaLength *
                            sin(_femur_angle + _tibia_angle -
                                std::numbers::pi_v<float> / 2.0f));
...
std::array<Vector2, HexapodConst::kLegNum> PhantomXMkII::InitMinLegPosXY()
    const {
  std::array<Vector2, HexapodConst::kLegNum> res;

  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    const float angle =
        kMovableCoxaAngleMin + PhantomXMkIIConst::kCoxaDefaultAngle[i];
...
std::array<Vector2, HexapodConst::kLegNum> PhantomXMkII::InitMaxLegPosXY()
    const {
  std::array<Vector2, HexapodConst::kLegNum> res;

  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    const float angle =
        kMovableCoxaAngleMax + PhantomXMkIIConst::kCoxaDefaultAngle[i];
    res[i] = Vector2(cos(angle), sin(angle)).GetNormalized();
  }
