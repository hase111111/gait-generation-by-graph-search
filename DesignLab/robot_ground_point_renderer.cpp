...
//! @file robot_ground_point_renderer.cpp
...
void RobotGroundPointRenderer::SetNodeAndSimulationEndNodeIndex(
    const std::vector<RobotStateNode>& node,
    ...
    for (int i = 0; i < HexapodConst::kLegNum; i++) {
      ground_point[i] = {
          converter_ptr_->ConvertLegToGlobalCoordinate(
              node[loaded_node_num_].leg_pos[i], i,
              node[loaded_node_num_].center_of_mass_global_coord,
              node[loaded_node_num_].posture, true),
...
void RobotGroundPointRenderer::Draw(const size_t draw_simulation_num,
                                    const bool draw_all_simulation) const {
  unsigned int color[6] = {kRightLegGroundPointColor, kRightLegGroundPointColor,
                           kRightLegGroundPointColor, kLeftLegGroundPointColor,
                           kLeftLegGroundPointColor,  kLeftLegGroundPointColor};

  unsigned int color_black[6] = {
      kRightLegGroundPointDarkColor, kRightLegGroundPointDarkColor,
      kRightLegGroundPointDarkColor, kLeftLegGroundPointDarkColor,
      kLeftLegGroundPointDarkColor,  kLeftLegGroundPointDarkColor};

  for (size_t i = 0; i < ground_point_.size(); i++) {
    for (auto& leg_data : ground_point_[i]) {
      for (size_t leg_index = 0; leg_index < HexapodConst::kLegNum;
           leg_index++) {
        ...
        if (draw_all_simulation || i == draw_simulation_num) {
          dxlib_util::DrawCube3DWithTopPos(
              dxlib_util::ConvertToDxlibVec(leg_data[leg_index].vec), 25,
              color[leg_index]);
        } else {
