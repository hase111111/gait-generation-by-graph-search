...
void CameraDragger::DraggedAction(const int cursor_dif_x,
                                  const int cursor_dif_y,
  ...
  if ((mouse_key_bit & MOUSE_INPUT_LEFT) &&
    ...
    if (mouse_move != 0) {
      ...
      camera_ptr_->SetCameraQuat(res);
             ...
             Squared(cursor_dif_x) + Squared(cursor_dif_y) >
                 Squared(kMouseMoveMargin)) {
    // 右クリックしていたらカメラの平行移動.
