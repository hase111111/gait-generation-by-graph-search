	...
	for (int x = 0; x < MapConst::LP_DIVIDE_NUM; x++)
	{
		for (int y = 0; y < MapConst::LP_DIVIDE_NUM; y++)
		{
			for (int i = 0; i < _map.getPointNumFromDevideMap(x,y); i++)
			{
				...
				if ((x + y) % 2 == 0) 
				{
					drawCube3D(convertToDxVec(_map.getPosFromDevideMap(x, y, i)), 15.0f, COLOR_GRAY);
				}
				else
				{
					drawCube3D(convertToDxVec(_map.getPosFromDevideMap(x, y, i)), 15.0f, COLOR_LIGHT_GRAY);
				}
