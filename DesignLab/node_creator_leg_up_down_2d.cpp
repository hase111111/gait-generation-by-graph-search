...
//! @file node_creator_leg_up_down_2d.cpp
...
void NodeCreatorLegUpDown2d::Create(
    const RobotStateNode& current_node, int current_node_index,
...
bool NodeCreatorLegUpDown2d::IsAbleLegPos(const RobotStateNode& node,
  ...
  // 脚位置4と比較して前か後ろか.
  if (node.leg_reference_pos[leg_index].ProjectedXY().Cross(
          node.leg_pos[leg_index].ProjectedXY()) *
