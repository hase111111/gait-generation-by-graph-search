...
//! 144Fpsや240Fps程度のゲーミングモニターを使っている場合，画面がおかしくなることがある．
//! @n よってこのクラスを使ってFPSを制御する．
//! FPSは Frames per Second：1秒当たりの画面更新回数のこと．First Person Shootingのことではない．
...
public:
    ...
    //! @return 処理が詰まって描画を飛ばしたいときに true を返す．その後フラグを false にする．
    bool SkipDrawScene();
...
private:
    ...
    //! リストに2秒分のフレームごとにかかった時間を記録するため，リストの最大サイズを決める．
    const int kListMax;

    //! 1フレームごとにかかった時間を記録するリスト．
    std::list<int> time_list_;

    //! コマ落ちを実装するためのフラグ．trueであれば 1フレーム描画を飛ばし，その後フラグを折る
