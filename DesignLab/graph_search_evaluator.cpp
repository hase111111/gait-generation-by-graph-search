...
//! @file graph_search_evaluator.cpp
...
GraphSearchEvaluator::GraphSearchEvaluator(
  ...
  // 評価方法のマップの要素数と,優先的に評価を行う Tag
  // のリストの要素数が一致していることを確認する.
  assert(evaluation_method_map_.size() == evaluation_priority_list_.size());

  //! @todo ここで,evaluation_priority_list_ のもつ Tag
  //! が,evaluation_method_map_ の Tag と一致していることを確認する.
  //! 面倒であるため,上記の assert で代用する.
}

bool GraphSearchEvaluator::LeftIsBetter(
    const GraphSearchEvaluationValue& left,
  ...
  for (const auto& tag : evaluation_priority_list_) {
    ...
    assert(0.0f <= evaluation_method.margin);  // margin は負の値にはできない.
...
bool GraphSearchEvaluator::LeftIsBetterWithTag(
    const GraphSearchEvaluationValue& left,
    const GraphSearchEvaluationValue& right, Tag tag,
    bool return_true_case_of_equal) const {
  // 可読性のため,変数に格納する.
  const float& left_value = left.value.at(tag);
  const float& right_value = right.value.at(tag);
...
GraphSearchEvaluationValue GraphSearchEvaluator::InitializeEvaluationValue()
    const {
  // 最低の評価になるように初期化する.
  ...
  for (const auto& tag : evaluation_priority_list_) {
    ...
    if (evaluation_method.is_lower_better) {
      ...
      evaluation_value.value[tag] =
          GraphSearchEvaluationValue::kMaxEvaluationValue;
    } else {
      // 評価値が大きいほど良い場合,最小の評価値を設定する.
      evaluation_value.value[tag] =
          GraphSearchEvaluationValue::kMinEvaluationValue;
    }
  }
