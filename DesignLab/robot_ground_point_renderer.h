...
class RobotGroundPointRenderer final {
 public:
  ...
  void SetNodeAndSimulationEndNodeIndex(
      const std::vector<RobotStateNode>& result_node,
 ...
 private:
  ...
  const unsigned int
      kRightLegGroundPointDarkColor;  //!< 現在のシミュレーション以外の色 (右足)

  const unsigned int
      kLeftLegGroundPointDarkColor;  //!< 現在のシミュレーション以外の色 (左足)
  ...
  std::vector<std::vector<std::array<VectorAndIsGround, HexapodConst::kLegNum>>>
      ground_point_;
};
