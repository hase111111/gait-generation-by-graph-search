...
//! @file node_creator_com_up_down.cpp
...
void NodeCreatorComUpDown::Create(
  ...
  if (map_.IsInMap(current_node.center_of_mass_global_coord)) {
    const int map_index_x =
        map_.GetDividedMapIndexX(current_node.center_of_mass_global_coord.x);
    const int map_index_y =
        map_.GetDividedMapIndexY(current_node.center_of_mass_global_coord.y);
  ...
  float highest_body_pos_z =
      map_highest_z + presenter_ptr_->GetGroundHeightMarginMax();
  float lowest_body_pos_z =
      map_highest_z + presenter_ptr_->GetGroundHeightMarginMin();
  ...
  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    ...
    if (leg_func::IsGrounded(current_node.leg_state, i)) {
      ...
      const float edge_b = current_node.leg_pos[i].ProjectedXY().GetLength() -
                           PhantomXMkIIConst::kCoxaLength;

      const float edge_a =
          sqrt(math_util::Squared(edge_c) - math_util::Squared(edge_b));

      // 接地脚の最大重心高さの中から一番小さいものを全体の最大重心位置として記録する._aは脚の接地点からどれだけ上げられるかを表しているので,グローバル座標に変更する.
      highest_body_pos_z =
          (std::min)(edge_a + current_node.center_of_mass_global_coord.z +
                         current_node.leg_pos[i].z,
                     highest_body_pos_z);
    }
  }
