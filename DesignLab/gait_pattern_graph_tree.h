...
class GaitPatternGraphTree final {
 public:
  ...
  explicit inline GaitPatternGraphTree(const int graph_max_size)
      : graph_size_(0), kGraphMaxSize(graph_max_size) {
  ...
  [[nodiscard]] constexpr bool HasRoot() const { return graph_size_ != 0; }
  ...
  [[nodiscard]] constexpr int GetRootIndex() const { return 0; }
  ...
  [[nodiscard]] const RobotStateNode& GetParentNode(const int index,
  ...
  [[nodiscard]] const int GetParentNodeIndex(const int index,
                                             const int depth) const;
  ...
  inline void AddNode(const RobotStateNode& node) {
    ...
    assert(nodes_[graph_size_ - 1].IsLootNode() ||
           nodes_[graph_size_ - 1].depth - 1 ==
               nodes_[nodes_[graph_size_ - 1].parent_index].depth);
  }
  ...
  [[nodiscard]] std::vector<float> GetCoMVerticalTrajectory(
    ...
    for (int i = index; i != -1; i = nodes_[i].parent_index) {
      if (com_vertical_trajectory_reverse.empty() ||
          com_vertical_trajectory_reverse.back() !=
              nodes_[i].center_of_mass_global_coord.z) {
        com_vertical_trajectory_reverse.push_back(
            nodes_[i].center_of_mass_global_coord.z);
      }
    }

    // 根ノードから現在のノードまでの重心の上下移動軌跡を取得する.
    std::vector<float> com_vertical_trajectory(
        com_vertical_trajectory_reverse.size());
    for (int i = 0; i < com_vertical_trajectory_reverse.size(); ++i) {
      com_vertical_trajectory[i] = com_vertical_trajectory_reverse
          [com_vertical_trajectory_reverse.size() - 1 - i];
    }
