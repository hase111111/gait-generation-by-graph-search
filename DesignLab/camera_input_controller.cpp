...
CameraInputController::CameraInputController(DxlibCamera* const  camera_manager) : camera_manager_{ camera_manager }
{
	assert(camera_manager_ != nullptr);
}
...
void CameraInputController::Activate(const std::shared_ptr<const Mouse> mouse_ptr)
{
	...
	{
		camera_manager_->AddCameraToTargetLength(kCameraZoomSpeed * mouse_ptr->GetWheelRot() * -1);
	}
	...
	if (mouse_ptr->GetPressingCount(MOUSE_INPUT_MIDDLE) > 0)
	{
		...
		if (abs(mouse_ptr->GetDiffPosX()) > 0)
		{
			...
			dl::Quaternion res = camera_manager_->GetCameraQuat() * move_quatx;

			res = res.GetNormalized();

			camera_manager_->SetCameraQuat(res);
		}
		...
		if (abs(mouse_ptr->GetDiffPosY()) > 0)
		{
			...
			dl::Quaternion res = camera_manager_->GetCameraQuat() * move_quaty;
	...
	else if (mouse_ptr->GetPressingCount(MOUSE_INPUT_LEFT) > 0)
	{
		...
		if (mouse_move != 0) 
		{
			...
			const dl::Quaternion res = (camera_manager_->GetCameraQuat() * move_quat).GetNormalized();
	...
	else if (mouse_ptr->GetPressingCount(MOUSE_INPUT_RIGHT) > 0 && mouse_ptr->GetDiffPos() > kMouseMoveMargin)
	{
		...
		if (camera_manager_->GetCameraViewMode() != CameraViewMode::kFreeControlledAndMovableTarget)
		{
			//表示モードを注視点を自由に動かせるモードに変更
			camera_manager_->SetCameraViewMode(CameraViewMode::kFreeControlledAndMovableTarget);
		}
		...
		if (abs(mouse_ptr->GetDiffPosX()) > 0)
		{
			...
			move_vec_x = dl::RotateVector3(move_vec_x, camera_manager_->GetCameraQuat());
		}
		...
		if (abs(mouse_ptr->GetDiffPosY()) > 0)
		{
			...
			move_vec_y = dl::RotateVector3(move_vec_y, camera_manager_->GetCameraQuat());
		}

		dl::Vector3 now_target_pos = camera_manager_->GetFreeTargetPos();	//現在のターゲット座標を取得

		now_target_pos = now_target_pos + move_vec_x + move_vec_y;			//移動量を加算

		camera_manager_->SetFreeTargetPos(now_target_pos);					//ターゲット座標を更新	
	}
}