...
//! @file node_creator_leg_up_down_radius.cpp
...
void NodeCreatorLegUpDownRadius::Create(
    ...
    if (is_able_leg_ground_pattern[i]) {
      ...
      for (int j = 0; j < HexapodConst::kLegNum; j++) {
        if (new_is_ground[j]) {
          ...
          res_node.leg_pos[j].z =
              presenter_ptr_->GetFreeLegPosLegCoordinate(j).z;
...
bool NodeCreatorLegUpDownRadius::IsGroundableLeg(
...
bool NodeCreatorLegUpDownRadius::IsAbleLegPos(const RobotStateNode& _node,
  ...
  if (discrete_leg_pos == DiscreteLegPos::kLowerFront ||
    ...
    if (_node.leg_reference_pos[leg_index].ProjectedXY().GetSquaredLength() -
            _node.leg_pos[leg_index].ProjectedXY().GetSquaredLength() -
            kLegMargin >
        0) {
      return true;
    }
    ...
    if (_node.leg_pos[leg_index].ProjectedXY().GetSquaredLength() -
            _node.leg_reference_pos[leg_index]
                .ProjectedXY()
                .GetSquaredLength() -
            kLegMargin >
        0) {
    ...
    // 脚位置4と同じくらい.
    if (std::abs(_node.leg_reference_pos[leg_index]
                     .ProjectedXY()
                     .GetSquaredLength() -
                 _node.leg_pos[leg_index].ProjectedXY().GetSquaredLength()) <
        kLegMargin) {
