...
//! @file dxlib_gui_node_displayer.cpp
...
DxlibGuiNodeDisplayer::DxlibGuiNodeDisplayer(
  ...
  button_.push_back(std::make_unique<SimpleButton>(
      "基本情報", 10 + button_size_x / 2,
      gui_top_pos_y_ + height_ - button_size_y / 2 - 10, button_size_x,
      button_size_y));
  ...
  button_.push_back(std::make_unique<SimpleButton>(
      "joint", (10 + button_size_x / 2) + (10 + button_size_x),
      gui_top_pos_y_ + height_ - button_size_y / 2 - 10, button_size_x,
      button_size_y));
  ...
  button_.push_back(std::make_unique<SimpleButton>(
      "脚先座標", (10 + button_size_x / 2) + (10 + button_size_x) * 2,
      gui_top_pos_y_ + height_ - button_size_y / 2 - 10, button_size_x,
      button_size_y));
  ...
  const int close_button_x =
      gui_left_pos_x_ + width_ - close_button_size / 2 - 2;
  const int close_button_y = gui_top_pos_y_ + close_button_size / 2 + 2;

  button_.push_back(
      std::make_unique<SimpleButton>("×", close_button_x, close_button_y,
...
void DxlibGuiNodeDisplayer::DrawNodeInfo() const {
  ...
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, " bit : %s", display_node_.leg_state.to_string().c_str());

  const auto com_pos_name = EnumToStringRemoveTopK(
  ...
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, " 重心 : %s(%d)", com_pos_name.c_str(), com_pos_num);
  ...
  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    ...
    if (i < HexapodConst::kLegNum / 2) {
      str_leg_pos_right +=
          std::format("{}-{}({}), ", leg_name[i], EnumToStringRemoveTopK(pos),
                      static_cast<int>(pos));
    } else {
      str_leg_pos_left +=
          std::format("{}-{}({}), ", leg_name[i], EnumToStringRemoveTopK(pos),
  ...
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, " 脚位置 : ", str_leg_pos_right.c_str());

  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, "  %s", str_leg_pos_right.c_str());

  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, "  %s", str_leg_pos_left.c_str());

  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, " 脚の状態：%s", str_ground.c_str());

  // 重心を表示する
  ++text_line;
  DrawFormatStringToHandle(text_pos_x,
                           text_pos_y_min + text_interval_y * (text_line++),
                           text_color, font_handle_, "重心位置");
  ...
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, "回転 (w:%5.3f,x:%5.3f,y:%5.3f,z:%5.3f)",
      display_node_.posture.w, display_node_.posture.v.x,
      display_node_.posture.v.y, display_node_.posture.v.z);
  ...
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, "　オイラー角(x:%5.3f[deg],y:%5.3f[deg],z:%5.3f[deg])",
  ...
  DrawFormatStringToHandle(text_pos_x,
                           text_pos_y_min + text_interval_y * (text_line++),
                           text_color, font_handle_, "脚位置");

  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    DrawFormatStringToHandle(
        text_pos_x, text_pos_y_min + text_interval_y * (text_line++),
        text_color, font_handle_, "%s %s", leg_name[i].c_str(),
  ...
  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    if (display_node_.leg_pos[i] == display_node_.leg_reference_pos[i]) {
      ...
      DrawFormatStringToHandle(
          text_pos_x, text_pos_y_min + text_interval_y * (text_line++),
          text_color_dark, font_handle_,
          " %s脚の基準座標(x:%5.3f,y:%5.3f,z:%5.3f)", leg_name[i].c_str(),
  ...
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, "深さ：%d, 次の動作 : %s", display_node_.depth,
      EnumToStringRemoveTopK(display_node_.next_move).c_str());

  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, "指定がなければ単位は長さが[mm],角度が[rad]");
}
...
void DxlibGuiNodeDisplayer::DrawJointInfo() const {
  ...
  if (!calculator_ptr_) {
    DrawFormatStringToHandle(text_pos_x, text_pos_y_min + text_interval_y * 0,
  ...
  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    if (joint_state_[i].joint_angle.size() != 3) {
      DrawFormatStringToHandle(
          text_pos_x, text_pos_y_min + text_interval_y * 0, text_color,
          font_handle_, "間接の計算ができていない,またはされていません.");
  ...
  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    ...
    DrawFormatStringToHandle(
        text_pos_x, text_pos_y_min + text_interval_y * (text_line++),
        text_color, font_handle_, "[%d] c %s[deg],f %s[deg],t %s[deg]", i,
    ...
    DrawFormatStringToHandle(
        text_pos_x, text_pos_y_min + text_interval_y * (text_line++),
        text_color, font_handle_, "    c %3.3f[mm],f %3.3f[mm],t %3.3f[mm]",
        coxa_to_femur.GetLength(), femur_to_tibia.GetLength(),
    ...
    if (checker_ptr_->IsLegInRange(
      ...
      DrawFormatStringToHandle(
          text_pos_x, text_pos_y_min + text_interval_y * (text_line++),
          text_color, font_handle_, "    近似値された可動域内にあります.");
    } else {
      DrawFormatStringToHandle(
          text_pos_x, text_pos_y_min + text_interval_y * (text_line++),
          error_text_color, font_handle_, "    近似値された可動域外です.");
    }

    std::string str = "";

    const float coxa_min_angle = PhantomXMkIIConst::kCoxaDefaultAngle[i] +
                                 PhantomXMkIIConst::kCoxaAngleMin;
    const float coxa_max_angle = PhantomXMkIIConst::kCoxaDefaultAngle[i] +
    ...
    if (!str.empty()) {
      ...
      if (str.size() > max_str_size) {
        str = str.substr(0, max_str_size);
      }

      DrawFormatStringToHandle(text_pos_x,
                               text_pos_y_min + text_interval_y * (text_line++),
                               error_text_color, font_handle_,
                               "    実際の可動域の外です. %s", str.c_str());
    } else {
      DrawFormatStringToHandle(
          text_pos_x, text_pos_y_min + text_interval_y * (text_line++),
          text_color, font_handle_, "    実際の可動域の内です.");
    }
  }
}
...
void DxlibGuiNodeDisplayer::DrawGlobalPosInfo() const {
  ...
  DrawFormatStringToHandle(text_pos_x,
                           text_pos_y_min + text_interval_y * (text_line++),
                           text_color, font_handle_, "脚先座標(ローカル座標)");
  ...
  DrawFormatStringToHandle(text_pos_x,
                           text_pos_y_min + text_interval_y * (text_line++),
                           text_color, font_handle_, "脚先座標(ロボット座標)");

  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    const std::string str =
        converter_ptr_->ConvertLegToRobotCoordinate(display_node_.leg_pos[i], i)
            .ToString();

    DrawFormatStringToHandle(
        text_pos_x, text_pos_y_min + text_interval_y * (text_line++),
        text_color, font_handle_, "%s %s", leg_name[i].c_str(), str.c_str());
  }

  ++text_line;
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle_, "脚先座標(グローバル座標)");
    ...
    DrawFormatStringToHandle(
        ...
        converter_ptr_
            ->ConvertLegToGlobalCoordinate(
                display_node_.leg_pos[i], i,
                display_node_.center_of_mass_global_coord,
                display_node_.posture, true)
            .ToString()
            .c_str());
  }
  ...
  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    DrawFormatStringToHandle(
        ...
        text_pos_y_min + text_interval_y * text_line, text_color, font_handle_,
        "%s %f", leg_name[i].c_str(),
