...
bool ComSelector::GetComFromPolygon(const Polygon2& polygon,
  ...
  std::pair<bool, Vector2>
      com_candidate[kDiscretizationNum * kDiscretizationNum];
  ...
  for (int i = 0; i < polygon.GetVertexNum(); ++i) {
    Vector2 edge = polygon.GetVertex(i) -
                   polygon.GetVertex((i + 1) % polygon.GetVertexNum());
  ...
  for (int i = 0; i < kDiscretizationNum * kDiscretizationNum; ++i) {
    ...
    const Vector3 dif =
        after_move_com_pos - current_node.center_of_mass_global_coord;
    const Vector3 real_dif =
        RotateVector3(dif, current_node.posture.GetConjugate());
...
bool ComSelector::MakeComCandidatePoint(
    ...
    std::pair<bool, Vector2> coms[kDiscretizationNum * kDiscretizationNum])
    const {
