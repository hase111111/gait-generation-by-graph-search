...
bool FileTree::SelectFile(const std::string& path, int max_depth,
  ...
  if (file_list.empty()) {
    cmdio::Print("The file did not exist.", OutputDetail::kSystem);
    return false;
  }
  ...
  while (true) {
    ...
    cmdio::OutputNewLine(1, OutputDetail::kSystem);
    cmdio::Print(
        "The selected file is " + file_list[static_cast<size_t>(select_index)],
...
FileTree::FileTreeData FileTree::MakeFileTree(const std::string& path,
                                              int max_depth,
                                              const std::string& extension,
  ...
  for (const auto& entry : std::filesystem::directory_iterator(path)) {
    if (entry.is_directory()) {
      ...
      if (max_depth == 0) {
        ...
        tree.directory.push_back(MakeFileTree(
            entry.path().string(), max_depth - 1, extension, keyword));
      }
      ...
      if (!keyword.empty()) {
        if (entry.path().filename().string().find(keyword) ==
            std::string::npos) {
          continue;
        }
      }
...
void FileTree::OutputFileTree(const FileTreeData& tree, int depth,
  ...
  if (!(not_display_empty && tree.file.empty() && tree.directory.empty())) {
    ...
    cmdio::Print(indent, OutputDetail::kSystem);

    std::string::size_type pos = tree.path.find_last_of("/\\");
    std::string dir_name = ((depth == 0) ? "" : "- ");
    dir_name +=
        std::string("[ ") + tree.path.substr(pos + 1) + std::string(" ]");

    cmdio::Print(indent + dir_name, OutputDetail::kSystem);
  }
  ...
  if (!tree.file.empty()) {
    ...
    for (const auto& file : tree.file) {
      cmdio::Print(
          indent + "|- " + file + " [-" + std::to_string(*file_count) + "-]",
          OutputDetail::kSystem);
...
std::vector<std::string> FileTree::MakeFileList(
    const FileTreeData& tree) const {
