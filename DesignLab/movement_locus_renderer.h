...
class MovementLocusRenderer final {
 public:
  ...
  void Draw(const size_t draw_simulation_num,
            bool draw_all_simulation = false) const;
 ...
 private:
  ...
  const float
      kLocusLineMaxLength;  //!< 軌跡の線の最大長さ.この値を上回る場合は表示しない.

  const float kLocusLineRadius;  //!< 軌跡の線の半径.

  std::vector<Vector3>
      move_locus_point_;  //! ロボットの動きの遷移を記録する配列.
