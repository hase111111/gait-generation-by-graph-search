 ...
 private:
  ...
  bool MakeComCandidatePoint(
      ...
      std::pair<bool, Vector2>
          output_coms[kDiscretizationNum * kDiscretizationNum]) const;

  //! @brief 絶対安全余裕を計算し,マージンを外れていないか調べる.
  bool IsInMargin(const Polygon2& polygon, const std::vector<Vector2>& edge_vec,
