...
public:
	...
	void Export(const std::string& file_path, const T& data)
	{
		...
		if (!ofs)
		{
			::designlab::cmdio::Output("設定ファイルの出力に失敗しました．file_path : " + file_path, OutputDetail::kSystem);
			return;
		}
		...
		::designlab::cmdio::Output("設定ファイルを出力しました．file_path : " + file_path, OutputDetail::kSystem);
	}
};
