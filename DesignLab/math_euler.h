...
struct EulerXYZ final {
  ...
  constexpr bool operator==(const EulerXYZ& other) const noexcept {
    ...
    return IsEqual(x_angle, other.x_angle) && IsEqual(y_angle, other.y_angle) &&
           IsEqual(z_angle, other.z_angle);
  }
  ...
  //! @param[in] z Z軸周りの回転.[deg]
  //! @return オイラー角.
  [[nodiscard]] static constexpr EulerXYZ MakeEulerXYZDeg(const float x,
                                                          const float y,
                                                          const float z) {
    using math_util::ConvertDegToRad;
    return EulerXYZ{ConvertDegToRad(x), ConvertDegToRad(y), ConvertDegToRad(z)};
  }
...
inline std::basic_ostream<Char>& operator<<(std::basic_ostream<Char>& os,
  ...
  os << FloatingPointNumToString(r.x_angle) << Char(',')
     << FloatingPointNumToString(r.y_angle) << Char(',')
     << FloatingPointNumToString(r.z_angle);
...
inline std::basic_istream<Char>& operator>>(std::basic_istream<Char>& is,
                                            EulerXYZ& r) {
  Char unused = 0;
  return is >> unused >> r.x_angle >> unused >> r.y_angle >> unused >>
         r.z_angle >> unused;
}
