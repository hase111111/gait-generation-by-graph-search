...
TEST_SUITE("string_util::Split") {
  ...
  TEST_CASE(
      "カンマで区切った時,カンマが取り除かれ区切られた文字列に分けられるべ"
      "き") {
    SUBCASE(
        "文字列「a,b,"
        "c」をカンマで区切った時,「a」「b」「c」に分けられるべき") {
...
TEST_SUITE("string_util::EnumToStringRemoveTopK") {
  enum class SampleEnum : int { kOne, knight, three, FOUR, Five };

  TEST_CASE(
      "先頭にkがついた列挙体の要素を渡した時,先頭のkを取り除いた要素名が返るべ"
      "き") {
  ...
  TEST_CASE(
      "先頭にkがついてない列挙体の要素を渡した時,そのままの要素名が返るべき") {
...
TEST_SUITE("string_util::EnumValuesToString") {
  ...
  TEST_CASE(
      "区切り文字をカンマにした時,文字列「kApple,kBanana,"
      "kTomato」が返るべき") {
...
TEST_SUITE("string_util::EnumEntriesToString") {
  ...
  TEST_CASE(
      "区切り文字をカンマにした時,文字列「kApple = 0,kBanana = 1,kTomato = "
      "5」が返るべき") {
