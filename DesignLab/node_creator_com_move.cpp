...
//! @file node_creator_com_move.cpp
...
void NodeCreatorComMove::Create(
  ...
  for (int i = 0; i < ComCandidatePolygonMaker::kMakePolygonNum; ++i) {
    ...
    if (!candidate_polygons[i].is_able) {
      continue;
    }

    Vector3 result_com;

    if (selector_.GetComFromPolygon(candidate_polygons[i].polygon, current_node,
                                    &result_com)) {
      ...
      leg_func::ChangeDiscreteComPos(candidate_polygons[i].com_pos,
                                     &next_node.leg_state);
      ...
      next_node.ChangeToNextNode(current_num,
                                 next_move_);  // 深さや親ノードを変更する
