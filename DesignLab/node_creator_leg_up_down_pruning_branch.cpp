...
//! @file node_creator_leg_up_down_pruning_branch.cpp
...
void NodeCreatorLegUpDownPruningBranch::Create(
  ...
  boost::dynamic_bitset<> is_able_leg_ground_pattern(
      com_func::GetLegGroundPatternNum());
    ...
    if (is_able_leg_ground_pattern[i]) {
      ...
      for (int j = 0; j < HexapodConst::kLegNum; j++) {
        if (new_is_ground[j]) {
          ...
          res_node.leg_reference_pos[j].x =
              presenter_ptr_->GetFreeLegPosLegCoordinate(j).x;
          res_node.leg_reference_pos[j].y =
              presenter_ptr_->GetFreeLegPosLegCoordinate(j).y;
        }
      }
...
bool NodeCreatorLegUpDownPruningBranch::IsGroundableLeg(
    const int now_leg_num, const RobotStateNode& current_node,
    Vector3* output_ground_pos) const {
  // 脚座標がマップのどこに当たるか調べて,そのマスの2つ上と2つ下の範囲内を全て探索する.
  const Vector3 global_leg_base_pos =
      converter_ptr_->ConvertLegToGlobalCoordinate(
          current_node.leg_reference_pos[now_leg_num], now_leg_num,
          current_node.center_of_mass_global_coord, current_node.posture, true);
      ...
      for (int n = 0; n < *pos_num; n++) {
        ...
        if (is_candidate_pos) {
          ...
          if (new_node.leg_reference_pos[now_leg_num].ProjectedXY().Cross(
              ...
              0 - MathConst<float>::kAllowableError) {
            continue;
          }
          ...
          // if ((candidate_pos -
          // new_node.leg_reference_pos[now_leg_num]).ProjectedXY().GetSquaredLength()
          // <
          //     (map_point_pos -
          //     new_node.leg_reference_pos[now_leg_num]).ProjectedXY().GetSquaredLength())
          //{
        ...
        // 脚が範囲外ならば追加せずに続行.
        if (!checker_ptr_->IsLegInRange(now_leg_num,
                                        new_node.leg_pos[now_leg_num])) {
          continue;
        }
...
bool NodeCreatorLegUpDownPruningBranch::IsAbleLegPos(
    const RobotStateNode& _node, const int leg_index) const {
  ...
  // 脚位置4と比較して前か後ろか.
  if (_node.leg_reference_pos[leg_index].ProjectedXY().Cross(
          _node.leg_pos[leg_index].ProjectedXY()) *
  ...
  if (discrete_leg_pos == DiscreteLegPos::kLowerFront ||
    ...
    // 脚位置4と同じくらい.
    if (std::abs(_node.leg_reference_pos[leg_index].z -
                 _node.leg_pos[leg_index].z) <= kHighMargin) {
      return true;
    }
  }
