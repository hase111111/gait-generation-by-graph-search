...
void Stopwatch::Start() { start_time_ = sc::system_clock::now(); }

void Stopwatch::End() { end_time_ = sc::system_clock::now(); }
...
double Stopwatch::GetElapsedSeconds() const {
  ...
  // ミリ秒に変換.
  const std::int64_t milli_sec =
      sc::duration_cast<sc::milliseconds>(dur).count();
