...
//! @subsubsection 参考
//! @li C++、constexprのまとめ
//! https://qiita.com/KRiver1/items/ef7731467b5ca83850cb ( アクセス日 2024/2/29
//! )
//! @li AB - 3.04.構造体>細かい話>コンストラクタ
//! https://atcoder.jp/contests/apg4b/tasks/APG4b_ab?lang=ja ( アクセス日
//! 2024/2/29 )
...
struct Vector3 final {
  ...
  constexpr bool operator==(const Vector3& v) const noexcept {
    if (math_util::IsEqual(v.x, x) && math_util::IsEqual(v.y, y) &&
  ...
  //! @return 距離.
  [[nodiscard]] inline float GetDistanceFrom(
  ...
  [[nodiscard]] constexpr bool IsZero() const noexcept {
    if (math_util::IsEqual(x, 0.0f) && math_util::IsEqual(y, 0.0f) &&
  ...
  [[nodiscard]] constexpr Vector2 ProjectedXY() const noexcept {
    return {x, y};
  }
  ...
  [[nodiscard]] constexpr static Vector3 GetFrontVec() noexcept {
    return {1, 0, 0};
  }
  ...
  [[nodiscard]] constexpr static Vector3 GetLeftVec() noexcept {
    return {0, 1, 0};
  }
  ...
  [[nodiscard]] constexpr static Vector3 GetUpVec() noexcept {
    return {0, 0, 1};
  }
  ...
  //! @return 零ベクトル.
  [[nodiscard]] constexpr static Vector3 GetZeroVec() noexcept {
    return {0, 0, 0};
  }
...
static_assert(math_util::IsEqual(Vector3::GetLeftVec().Dot(Vector3::GetUpVec()),
                                 0.f),
              "The dot product of LeftVec and UpVec is not zero.");

static_assert(math_util::IsEqual(Vector3::GetFrontVec().GetSquaredLength(),
                                 1.f),
