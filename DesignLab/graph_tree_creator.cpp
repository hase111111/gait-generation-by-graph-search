...
void GraphTreeCreator::Init(const DividedMapState& map_state) {
  ...
  node_creator_map_ = node_creator_builder_ptr_->Build(map_state);
}
...
nostd::expected<bool, std::string> GraphTreeCreator::CreateGraphTree(
  ...
  while (cnt < graph->GetGraphSize()) {
    ...
    if (start_depth <= graph->GetNode(cnt).depth &&
        graph->GetNode(cnt).depth < max_depth) {
      std::vector<RobotStateNode>
          res_vec;  // cnt 番目のノードの子ノードを入れるベクター.

      MakeNewNodesByCurrentNode(graph->GetNode(cnt), cnt,
                                &res_vec);  // 子ノードを生成する.
...
void GraphTreeCreator::MakeNewNodesByCurrentNode(
  ...
  if (node_creator_map_.count(current_node.next_move) > 0) {
    node_creator_map_.at(current_node.next_move)
        ->Create(current_node, current_num, output_graph);

    return;
  } else {
    node_creator_map_.begin()->second->Create(current_node, current_num,
                                              output_graph);
