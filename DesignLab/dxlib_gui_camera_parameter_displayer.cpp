...
//! @file dxlib_gui_camera_parameter_displayer.cpp
...
void DxlibGuiCameraParameterDisplayer::DrawCameraParameter() const {
  ...
  DrawFormatStringToHandle(
      ...
      font_handle, "カメラの向き(正規化クォータニオン)");
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle, "　(w:%5.3f,x:%5.3f,y:%5.3f,z:%5.3f)",
      camera_ptr_->GetNowCameraQuat().w, camera_ptr_->GetNowCameraQuat().v.x,
      camera_ptr_->GetNowCameraQuat().v.y, camera_ptr_->GetNowCameraQuat().v.z);
  ...
  DrawFormatStringToHandle(text_pos_x,
                           ...
                           text_color, font_handle, "カメラの向き(オイラー角)");
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle, "　(roll:%5.3f[deg],pitch:%5.3f[deg],yaw:%5.3f[deg])",
      math_util::ConvertRadToDeg(euler_xyz.x_angle),
      math_util::ConvertRadToDeg(euler_xyz.y_angle),
      math_util::ConvertRadToDeg(euler_xyz.z_angle));

  text_line++;
  DrawFormatStringToHandle(text_pos_x,
                           text_pos_y_min + text_interval_y * (text_line++),
                           text_color, font_handle, "カメラの表示モード");
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle, "　%s",
      static_cast<std::string>(
          magic_enum::enum_name(camera_ptr_->GetCameraViewMode()))
          .c_str());

  DrawFormatStringToHandle(text_pos_x,
                           text_pos_y_min + text_interval_y * (text_line++),
                           text_color, font_handle, "カメラの注視点の座標");
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle, "　(x:%5.3f [mm],y:%5.3f [mm],z:%5.3f [mm])",
      camera_ptr_->GetNowTargetPos().x, camera_ptr_->GetNowTargetPos().y,
      camera_ptr_->GetNowTargetPos().z);

  DrawFormatStringToHandle(text_pos_x,
                           text_pos_y_min + text_interval_y * (text_line++),
                           text_color, font_handle, "カメラと対象との距離");
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle, "　%5.3f [mm]", camera_ptr_->GetNowCameraToTargetLength());

  text_line++;
  DrawFormatStringToHandle(text_pos_x,
                           text_pos_y_min + text_interval_y * (text_line++),
                           text_color, font_handle, "カメラの位置");
  DrawFormatStringToHandle(
      text_pos_x, text_pos_y_min + text_interval_y * (text_line++), text_color,
      font_handle, "　(x:%5.3f [mm],y:%5.3f [mm],z:%5.3f [mm])",
      camera_ptr_->GetNowCameraPos().x, camera_ptr_->GetNowCameraPos().y,
      camera_ptr_->GetNowCameraPos().z);
}
