...
//! true_typeを継承する.
template <typename T>
struct has_input_operator<T, decltype(std::declval<std::istream&>() >>
                                          std::declval<T&>(),
                                      std::declval<void>())>
...
//! true_typeを継承する.
template <typename T>
struct has_output_operator<T, decltype(std::declval<std::ostream&>()
                                           << std::declval<T&>(),
                                       std::declval<void>())>
    : public std::true_type {};
...
static_assert(is_toml11_available_type_vector_type<int*>::value == false,
              "int* type is not a std::vector type.");
static_assert(is_toml11_available_type_vector_type<std::vector<int>>::value ==
                  true,
              "The type std::vector<int> is a type of std::vector and can used "
              "in toml11.");
static_assert(
...
static_assert(is_toml11_available_type<std::vector<int>>::value == true,
              "The type std::vector<int> is a type of std::vector and can be "
              "used in toml11.");
static_assert(is_toml11_available_type<std::vector<int*>>::value == false,
              "type std::vector<*int> is a std::vector type, but it is not a "
              "usable type.");
...
//! true_typeを継承する.
template <typename T>
struct is_vector_of_has_output_operator<std::vector<T>>
