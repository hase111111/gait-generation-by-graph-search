...
XrR1::XrR1(const XrR1ParameterRecord& parameter_record)
    : kBodyLiftingHeightMin(parameter_record.body_lifting_height_min),
      ...
      free_leg_pos_leg_coordinate_({{
          {160 * cos(XrR1Const::kCoxaDefaultAngle[0]),
           160 * sin(XrR1Const::kCoxaDefaultAngle[0]), kFreeLegHeight},
          {160 * cos(XrR1Const::kCoxaDefaultAngle[1]),
           160 * sin(XrR1Const::kCoxaDefaultAngle[1]), kFreeLegHeight},
          {160 * cos(XrR1Const::kCoxaDefaultAngle[2]),
           160 * sin(XrR1Const::kCoxaDefaultAngle[2]), kFreeLegHeight},
          {160 * cos(XrR1Const::kCoxaDefaultAngle[3]),
           160 * sin(XrR1Const::kCoxaDefaultAngle[3]), kFreeLegHeight},
          {160 * cos(XrR1Const::kCoxaDefaultAngle[4]),
           160 * sin(XrR1Const::kCoxaDefaultAngle[4]), kFreeLegHeight},
          {160 * cos(XrR1Const::kCoxaDefaultAngle[5]),
           160 * sin(XrR1Const::kCoxaDefaultAngle[5]), kFreeLegHeight},
      }}),
      leg_base_pos_robot_coordinate_({{
          {XrR1Const::kCoxaBaseOffsetX, -XrR1Const::kCoxaBaseOffsetY,
           XrR1Const::kCoxaBaseOffsetZ},  // 脚0 右上.
          {0.0f, -XrR1Const::kCenterCoxaBaseOffsetY,
           XrR1Const::kCoxaBaseOffsetZ},  // 脚1 右横.
          {-XrR1Const::kCoxaBaseOffsetX, -XrR1Const::kCoxaBaseOffsetY,
           XrR1Const::kCoxaBaseOffsetZ},  // 脚2 右下.
          {-XrR1Const::kCoxaBaseOffsetX, XrR1Const::kCoxaBaseOffsetY,
           XrR1Const::kCoxaBaseOffsetZ},  // 脚3 左下.
          {0.0f, XrR1Const::kCenterCoxaBaseOffsetY,
           XrR1Const::kCoxaBaseOffsetZ},  // 脚4 左横.
          {XrR1Const::kCoxaBaseOffsetX, XrR1Const::kCoxaBaseOffsetY,
           XrR1Const::kCoxaBaseOffsetZ},  // 脚5 左上.
      }}),
...
HexapodJointState XrR1::CalculateJointState(
  ...
  // femur jointの計算.
  {
    const Vector3 femur_joint_pos =
        Vector3{XrR1Const::kCoxaLength * std::cos(res.joint_angle[0]),
                XrR1Const::kCoxaLength * std::sin(res.joint_angle[0]), 0};
    ...
    if (!math_util::CanMakeTriangle((leg_pos - femur_joint_pos).GetLength(),
      ...
      const Vector3 candidate_leg_pos =
          femur_joint_pos +
          Vector3{(XrR1Const::kFemurLength + XrR1Const::kTibiaLength) *
      ...
      const Vector3 candidate_leg_pos_phase =
          femur_joint_pos +
          Vector3{(XrR1Const::kFemurLength * std::cos(angle_ft_phase) +
      ...
      res.joint_pos_leg_coordinate[2] =
          femur_joint_pos +
          Vector3{XrR1Const::kFemurLength * std::cos(angle_f) *
                      std::cos(res.joint_angle[0]),
      ...
      res.joint_pos_leg_coordinate[3] =
          res.joint_pos_leg_coordinate[2] +
          Vector3{XrR1Const::kTibiaLength * std::cos(angle_f + angle_t) *
                      std::cos(res.joint_angle[0]),
  ...
  // tibia jointの計算.
  {
    const Vector3 femur_to_tibia =
        Vector3{XrR1Const::kFemurLength * std::cos(res.joint_angle[0]) *
  ...
  // tibia angleの計算.
  {
    const float tibia_angle =
        std::atan2((femur_to_leg_z -
                    XrR1Const::kFemurLength * std::sin(res.joint_angle[1])),
                   (femur_to_leg_x -
                    XrR1Const::kFemurLength * std::cos(res.joint_angle[1]))) -
        res.joint_angle[1];
...
Vector3 XrR1::ConvertGlobalToLegCoordinate(const Vector3& converted_position,
                                           const int leg_index,
...
Vector3 XrR1::ConvertLegToGlobalCoordinate(const Vector3& converted_position,
                                           int leg_index,
                                           const Vector3& center_of_mass_global,
                                           const Quaternion& robot_quat,
                                           const bool consider_rot) const {
...
bool XrR1::IsBodyInterferingWithGround(
    const RobotStateNode& node, const DividedMapState& divided_map) const {
  // 重心の干渉を調べる.
  ...
  for (int i = 0; i < HexapodConst::kLegNum; ++i) {
    ...
    if (divided_map.IsInMap(coxa_pos_global_coord)) {
      const float coxa_top_z = (std::max)(
          divided_map.GetTopZ(
              divided_map.GetDividedMapIndexX(coxa_pos_global_coord.x),
              divided_map.GetDividedMapIndexY(coxa_pos_global_coord.y)),
          divided_map.GetMapMinZ());
...
std::array<float, XrR1::kMaxLegRSize> XrR1::InitMaxLegR() const {
  ...
  for (int iz = 0; iz < 200; iz++) {
    for (int ix = 53; ix < 248; ix++) {
      ...
      // im = 0 だと定義域エラー.
      const float _q2 = acos((math_util::Squared(XrR1Const::kFemurLength) +
      ...
      const float _tibia_angle =
          acos((math_util::Squared(XrR1Const::kFemurLength) +
                math_util::Squared(XrR1Const::kTibiaLength) -
                math_util::Squared(_im)) /
      ...
      const float _K_trueX =
          XrR1Const::kFemurLength * cos(_femur_angle) +
          XrR1Const::kTibiaLength * cos(_femur_angle + _tibia_angle -
                                        std::numbers::pi_v<float> / 2.0f);
      ...
      _kinematics.z =
          -(XrR1Const::kFemurLength * sin(_femur_angle) +
            XrR1Const::kTibiaLength * sin(_femur_angle + _tibia_angle -
                                          std::numbers::pi_v<float> / 2.0f));
