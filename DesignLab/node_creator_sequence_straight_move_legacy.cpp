...
//! @file node_creator_sequence_straight_move_legacy.cpp
...
NodeCreatorSequenceStraightMoveLegacy::Build(const DividedMapState& map) const {
  using enum DiscreteLegPos;
  using enum HexapodMove;

  std::map<HexapodMove, std::unique_ptr<INodeCreator>> node_creator;

  const auto hierarchy_list =
      std::vector<DiscreteLegPos>{kBack, kCenter, kFront};
  ...
  node_creator[kComUpDown] = std::make_unique<NodeCreatorComMoveStraight>(
