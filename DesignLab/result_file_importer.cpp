...
bool ResultFileImporter::ImportNodeListAndMapState(
  ...
  if (!fs::exists(file_path)) {
    cmdio::ErrorOutput("The 'NodeList' file did not exist.");
    return false;
  }
  ...
  map_file_path.replace(map_file_path.find(ResultFileConst::kNodeListName),
                        ResultFileConst::kNodeListName.size(),
                        ResultFileConst::kMapStateName);

  if (!fs::exists(map_file_path)) {
    cmdio::ErrorOutput("The MapState file did not exist.");
    return false;
  }

  if (!ImportNodeList(file_path, node_list) ||
      !ImportMapState(map_file_path, map_state)) {
    cmdio::ErrorOutput("An error occurred while loading the file.");
  }
...
bool ResultFileImporter::ImportNodeList(
  ...
  if (!ifs.is_open()) {
    cmdio::SystemOutput("Could not open the file.");
...
bool ResultFileImporter::ImportMapState(const std::string& file_path,
                                        MapState* map_state) const {
  assert(map_state != nullptr);
  ...
  for (const auto& i : str_list) {
    ...
    try {
      ...
      cmdio::OutputF(OutputDetail::kWarning,
