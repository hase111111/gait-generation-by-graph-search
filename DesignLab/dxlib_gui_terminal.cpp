...
DxlibGuiTerminal::DxlibGuiTerminal(
  ...
  for (int i = 0; i < gui_list_.size(); ++i) {
    ...
    // typeidの返り値は"class
    // DxlibGui～"という文字列になっているので,邪魔な文字を削除する.
    const std::vector<std::string> unnecessary_words = {"DxlibGui", " ",
                                                        "const", "*", "&"};

    for (const auto& word : unnecessary_words) {
      // そもそも文字列に word が含まれていない場合は何もしない
      if (class_name.find(word) == std::string::npos) {
        continue;
      }
    ...
    button_list_.push_back(std::make_shared<SimpleButton>(
        class_name, kLeftTopX + kButtonSize + i * 100,
        kLeftTopY + kTerminalHeight / 2, kButtonSize, kButtonSize));

    button_list_.back()->SetActivateFunction(
        [this, i]() { gui_list_[i]->SetVisible(true); });
...
bool DxlibGuiTerminal::CursorOnGui(int cursor_x, int cursor_y) const noexcept {
  if (is_closed_) {
    return cursor_x >= kLeftTopX &&
           cursor_x <= kLeftTopX + kClosedTerminalWidth &&
...
void DxlibGuiTerminal::DrawClosedTerminal() const {
  ...
  DrawBox(kLeftTopX - frame_width, kLeftTopY - frame_width,
          kLeftTopX + closed_box_width,
          kLeftTopY + kTerminalHeight + frame_width, frame_color, TRUE);
  DrawTriangleAA(kLeftTopX + closed_box_width, kLeftTopY - frame_width,
                 kLeftTopX + closed_box_width,
                 kLeftTopY + kTerminalHeight + frame_width,
                 kLeftTopX + kClosedTerminalWidth + frame_width,
                 kLeftTopY + kTerminalHeight / 2, frame_color, TRUE);

  DrawBox(kLeftTopX, kLeftTopY, kLeftTopX + closed_box_width,
          kLeftTopY + kTerminalHeight, base_color, TRUE);
  DrawTriangleAA(kLeftTopX + closed_box_width, kLeftTopY,
                 kLeftTopX + closed_box_width, kLeftTopY + kTerminalHeight,
                 kLeftTopX + kClosedTerminalWidth,
                 kLeftTopY + kTerminalHeight / 2, base_color, TRUE);
...
void DxlibGuiTerminal::DrawTerminal() const {
  ...
  DrawBox(kLeftTopX - frame_width, kLeftTopY - frame_width,
          kLeftTopX + closed_box_width,
          kLeftTopY + kTerminalHeight + frame_width, frame_color, TRUE);
  DrawTriangleAA(static_cast<float>(kLeftTopX + closed_box_width),
                 static_cast<float>(kLeftTopY - frame_width),
                 static_cast<float>(kLeftTopX + closed_box_width),
                 static_cast<float>(kLeftTopY + kTerminalHeight + frame_width),
                 static_cast<float>(kLeftTopX + kTerminalWidth + frame_width),
  ...
  DrawTriangleAA(static_cast<float>(kLeftTopX + closed_box_width),
                 ...
                 static_cast<float>(kLeftTopX + kTerminalWidth),
                 static_cast<float>(kLeftTopY + kTerminalHeight / 2),
                 base_color, TRUE);
...
void DxlibGuiTerminal::DrawButtonGuard() const {
  for (int i = 0; i < button_list_.size(); ++i) {
    if (gui_list_[i]->IsVisible()) {
      ...
      DrawBox(button_list_[i]->GetPosMiddleX() - kButtonSize / 2,
              button_list_[i]->GetPosMiddleY() - kButtonSize / 2,
              button_list_[i]->GetPosMiddleX() + kButtonSize / 2,
              button_list_[i]->GetPosMiddleY() + kButtonSize / 2, color, TRUE);
