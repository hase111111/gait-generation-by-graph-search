...
// boost bimapのために比較演算子を定義している．std名前空間の変更は非推奨のため，変更可能であれば変更する．
template <size_t N>
...
//! @brief 脚の接地パターンを表す型．leftがbitのデータ，rightがint型の番号．
//! @n bimaps::bimapは，左右の型の両方からアクセスできるmap．key→value，value→keyの両方が可能．
//! @n 前述の通り脚の接地パターンは36通りある．
//! @n それぞれの接地パターンに対応する数値を割り振って管理をしやすくするために，bimapを用いている．
...
//! @param [in,out] output 脚接地パターンが実行可能ならばtrue，不可能ならfalseにしたbool型の配列．
//! @n 引数で指定した脚が遊脚できない場合に取り得ない接地パターンをfalseに変更する．
