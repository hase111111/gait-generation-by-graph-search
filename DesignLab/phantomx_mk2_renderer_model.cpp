...
//! @file phantomx_mk2_renderer_model.cpp
...
void PhantomXMkIIRendererModel::DrawBody() const {
  ...
  const VECTOR scale =
      VGet(10.f, 10.f, 10.f);  // モデルの寸法を調整するためのスケール.

  MV1SetScale(body_model_handle, scale);

  // dxlib の座標系は左手座標系なので,右手座標系に変換するために逆転させる.
  MV1SetRotationMatrix(body_model_handle,
                       dxlib_util::ConvertToDxlibMat(ToRotationMatrix(
                           draw_node_.posture.ToLeftHandCoordinate())));

  MV1SetPosition(
      body_model_handle,
      dxlib_util::ConvertToDxlibVec(draw_node_.center_of_mass_global_coord));

  MV1DrawModel(body_model_handle);

  // 胴体の正面に目印のために球を描画する.
  const VECTOR sphere_pos = dxlib_util::ConvertToDxlibVec(
      RotateVector3({PhantomXMkIIConst::kCoxaBaseOffsetX - 20.f, 0.f, 0.f},
                    draw_node_.posture) +
  ...
  DrawSphere3D(sphere_pos, sphere_radius, sphere_div_num, sphere_color,
               spec_color, TRUE);
}
...
void PhantomXMkIIRendererModel::DrawCoxaLink(const int leg_index) const {
  ...
  const VECTOR coxa_joint_pos_global = dxlib_util::ConvertToDxlibVec(
      converter_ptr_->ConvertLegToGlobalCoordinate(
          draw_joint_state_[leg_index].joint_pos_leg_coordinate[0], leg_index,
          draw_node_.center_of_mass_global_coord, draw_node_.posture, true));

  const Quaternion coxa_quat =
      Quaternion::MakeByAngleAxis(math_util::ConvertDegToRad(-90.0f),
                                  Vector3::GetLeftVec()) *
      Quaternion::MakeByAngleAxis(draw_joint_state_[leg_index].joint_angle[0],
                                  Vector3::GetUpVec()) *
      draw_node_.posture.ToLeftHandCoordinate();
  ...
  // dxlib の座標系は左手座標系なので,右手座標系に変換するために逆転させる.
  MV1SetRotationMatrix(coxa_model_handle, dxlib_util::ConvertToDxlibMat(
                                              ToRotationMatrix(coxa_quat)));
...
void PhantomXMkIIRendererModel::DrawFemurLink(const int leg_index) const {
  const int thign_model_handle =
      ModelLoader::GetIns()->GetModelHandle("model/thign_l.mv1");

  if (thign_model_handle == -1) {
    printfDx("モデルの読み込みに失敗しました.(thign_model_handle)");
  }
  ...
  const VECTOR femur_joint_pos_global_coord = dxlib_util::ConvertToDxlibVec(
      converter_ptr_->ConvertLegToGlobalCoordinate(
          draw_joint_state_[leg_index].joint_pos_leg_coordinate[1], leg_index,
          draw_node_.center_of_mass_global_coord, draw_node_.posture, true));
  ...
  const Quaternion thign_def_quat =
      ...
      Quaternion::MakeByAngleAxis(virtual_link_offset_angle,
                                  Vector3::GetLeftVec()) *
      Quaternion::MakeByAngleAxis(coxa_angle, Vector3::GetUpVec());

  // 原点の位置が少しズレているので,補正する.
  const VECTOR offset_pos = dxlib_util::ConvertToDxlibVec(RotateVector3(
      Vector3::GetFrontVec(),
      Quaternion::MakeByAngleAxis(coxa_angle, Vector3::GetUpVec()) *
          draw_node_.posture.ToLeftHandCoordinate()));
  ...
  // dxlib の座標系は左手座標系なので,右手座標系に変換するために逆転させる.
  Quaternion thign_quat =
      thign_def_quat * draw_node_.posture.ToLeftHandCoordinate();
  MV1SetRotationMatrix(thign_model_handle, dxlib_util::ConvertToDxlibMat(
                                               ToRotationMatrix(thign_quat)));
...
void PhantomXMkIIRendererModel::DrawTibiaLink(const int leg_index) const {
  ...
  int tibia_model_handle =
      ModelLoader::GetIns()->GetModelHandle("model/tibia_l_fixed.mv1");
  ...
  const VECTOR tibia_joint_pos_global_coord = dxlib_util::ConvertToDxlibVec(
      converter_ptr_->ConvertLegToGlobalCoordinate(
          draw_joint_state_[leg_index].joint_pos_leg_coordinate[2], leg_index,
          draw_node_.center_of_mass_global_coord, draw_node_.posture, true));
  ...
  const Quaternion default_quat =
      ...
      Quaternion::MakeByAngleAxis(math_util::ConvertDegToRad(90.0f),
                                  Vector3::GetLeftVec()) *
      Quaternion::MakeByAngleAxis(-femur_angle - tibia_angle,
                                  Vector3::GetLeftVec()) *
      Quaternion::MakeByAngleAxis(math_util::ConvertDegToRad(-90.0f),
                                  Vector3::GetLeftVec()) *
      Quaternion::MakeByAngleAxis(coxa_angle, Vector3::GetUpVec()) *
  ...
  // dxlib の座標系は左手座標系なので,右手座標系に変換するために逆転させる.
  const Quaternion tibia_quat =
      default_quat * draw_node_.posture.ToLeftHandCoordinate();
  MV1SetRotationMatrix(tibia_model_handle, dxlib_util::ConvertToDxlibMat(
                                               ToRotationMatrix(tibia_quat)));
...
void PhantomXMkIIRendererModel::DrawJointAxis(const int leg_index) const {
  ...
  // Coxa の回転軸.
  {
    ...
    const VECTOR axis_vec = dxlib_util::ConvertToDxlibVec(RotateVector3(
        Vector3::GetUpVec() * axis_length / 2, draw_node_.posture));

    DrawCapsule3D(coxa_joint_pos_global_coord - axis_vec,
                  coxa_joint_pos_global_coord + axis_vec, axis_radius,
                  axis_div_num, coxa_axis_color, spec_color, TRUE);
  }
  ...
  // Femurの回転軸.
  {
    ...
    const VECTOR axis_vec = dxlib_util::ConvertToDxlibVec(
        RotateVector3(Vector3::GetLeftVec() * axis_length / 2,
                      draw_node_.posture * default_quat));

    DrawCapsule3D(femur_joint_pos_global_coord - axis_vec,
                  femur_joint_pos_global_coord + axis_vec, axis_radius,
                  axis_div_num, femur_axis_color, spec_color, TRUE);
  }
  ...
  // Tibiaの回転軸.
  {
    ...
    DrawCapsule3D(tibia_joint_pos_global_coord - axis_vec,
                  tibia_joint_pos_global_coord + axis_vec, axis_radius,
                  axis_div_num, tibia_axis_color, spec_color, TRUE);
  }
}
