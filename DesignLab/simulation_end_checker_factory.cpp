...
//! @file simulation_end_checker_factory.cpp
...
std::unique_ptr<ISimulationEndChecker> SimulationEndCheckerFactory::Create(
  ...
  if (record.end_check_mode == kGoalTape) {
    auto simulation_end_checker =
        std::make_unique<SimulationEndCheckerByGoalTape>(
            record.goal_tape_position_x);

    return std::move(simulation_end_checker);
  } else if (record.end_check_mode == kPosture) {
    EulerXYZ target_euler_rad{ConvertDegToRad(record.target_posture.x_angle),
                              ConvertDegToRad(record.target_posture.y_angle),
                              ConvertDegToRad(record.target_posture.z_angle)};

    auto simulation_end_checker =
        std::make_unique<SimulationEndCheckerByPosture>(
            ToQuaternion(target_euler_rad),
            ConvertDegToRad(record.target_posture_allowable_error_deg));
    ...
    auto simulation_end_checker =
        std::make_unique<SimulationEndCheckerByPosition>(
