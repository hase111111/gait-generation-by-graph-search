...
class DividedMapState final {
 public:
  //!< 1つのマスに存在する脚設置可能点の数は kDividedMapPointNum ×
  //!< kDividedMapPointNum 個.
  static constexpr int kDividedMapPointNum{4};

  //!< 1つのマスの一辺の長さ.
  static constexpr float kDividedAreaLength{MapState::kMapPointDistance *
                                            kDividedMapPointNum};

  static constexpr int kDividedNum{15};  //!< 格子の数.

  //! マップの最大のX座標.
  static constexpr float kDividedMapMaxX{kDividedAreaLength * kDividedNum /
                                         2.0f};
  ...
  //! マップの最大のY座標.
  static constexpr float kDividedMapMaxY{kDividedAreaLength * kDividedNum /
                                         2.0f};
  ...
  constexpr int GetDividedMapIndexX(const float pos_x) const noexcept {
    return static_cast<int>((pos_x - global_robot_com_.x - kDividedMapMinX) *
                            static_cast<float>(kDividedNum) /
  ...
  constexpr int GetDividedMapIndexY(const float pos_y) const noexcept {
    return static_cast<int>((pos_y - global_robot_com_.y - kDividedMapMinY) *
                            static_cast<float>(kDividedNum) /
  ...
  static constexpr int ClampDividedMapIndex(const int index) noexcept {
    ...
    if (kDividedNum <= index) {
      return kDividedNum - 1;
    }
  ...
  void GetPointVector(int x_index, int y_index,
 ...
 private:
  ...
  //! @n
  //! そのため,2次元の配列のインデックスを1次元の配列のインデックスに変換する.
  //! @param[in] x_index x座標,切り分けられたタイルの位置で指定する.
  //! @param[in] y_index y座標,切り分けられたタイルの位置で指定する.
  //! @return 1次元の配列のインデックス.
  constexpr int GetDividedMapIndex(const int x_index,
                                   const int y_index) const noexcept {
