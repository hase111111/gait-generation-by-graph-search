...
LegStateBit MakeLegStateBit(
    ...
    const std::array<DiscreteLegPos, HexapodConst::kLegNum>&
        discretized_leg_pos) {
  LegStateBit res = 0;

  // bitに変換する.
  LegStateBit discrete_com_pos_bit =
      static_cast<unsigned int>(discrete_com_pos);
  res |= discrete_com_pos_bit
         << kShiftToComNum;  // 重心パターンの数値だけビットを立てる.

  for (int i = 0; i < HexapodConst::kLegNum; i++) {
    // 接地しているならば上位ビットを立てる.
    size_t ground_bit_index = static_cast<size_t>(i + 1) * 4 - 1;
    if (is_ground[i]) {
      res[ground_bit_index] = true;
    }

    // 脚のビットを立てる.
    LegStateBit discrete_leg_pos_bit =
        static_cast<unsigned int>(discretized_leg_pos[i]);
    size_t shift_num = static_cast<size_t>(i) * 4;  // 4bitずつずらす

    res |= discrete_leg_pos_bit
           << shift_num;  // 脚の位置の数値だけビットを立てる
  }
...
void GetGroundedLegIndexByVector(const LegStateBit& leg_state,
...
void GetLiftedLegIndexByVector(const LegStateBit& leg_state,
                               std::vector<int>* res_index) {
  // res_index は nullptr でないこと,かつ空である必要がある.
  assert(res_index != nullptr);
  assert((*res_index).size() == 0);
...
DiscreteLegPos GetDiscreteLegPos(const LegStateBit& leg_state,
                                 const int leg_index) {
  // leg_indexは0～5の範囲にある必要がある.
  assert(0 <= leg_index);
  assert(leg_index < HexapodConst::kLegNum);
...
enums::DiscreteComPos GetDiscreteComPos(const LegStateBit& leg_state) {
  ...
  const int res = static_cast<int>(
      ((leg_state & kComStateMaskBit) >> kShiftToComNum).to_ulong());

  return static_cast<enums::DiscreteComPos>(res);
}

void ChangeLegState(const int leg_index,
                    const DiscreteLegPos new_discretized_leg_pos,
                    const bool is_ground, LegStateBit* leg_state) {
  // leg_indexは0～5の範囲にある必要がある.
  assert(0 <= leg_index);
  assert(leg_index < HexapodConst::kLegNum);
...
void ChangeDiscreteLegPos(const int leg_index,
                          const DiscreteLegPos new_discretized_leg_pos,
  ...
  // 新しい脚状態を生成する.
  const size_t shift_num =
      static_cast<size_t>(leg_index) * 4;  // 4bitずつずらす.

  // 4bitのデータを変更する地点までマスクをずらす.
  const LegStateBit mask = kLegPosMaskBit << shift_num;

  // bitに変換する.
  const LegStateBit discreate_leg_pos_bit =
      static_cast<unsigned int>(new_discretized_leg_pos);
  ...
  // 参考 : https://qiita.com/vivisuke/items/bc707190e008551ca07f (アクセス日
  // 2023/12/27)
  LegStateBit res = ((*leg_state) ^ state) & mask;
  (*leg_state) ^= res;
}

void ChangeGround(const int leg_index, const bool is_ground,
                  LegStateBit* leg_state) {
  // leg_indexは0～5の範囲にある必要がある.
  assert(0 <= leg_index);
  assert(leg_index < HexapodConst::kLegNum);
...
void ChangeAllLegGround(const LegGroundedBit& is_ground_list,
                        LegStateBit* leg_state) {
  // leg_state は nullptrではない.
  assert(leg_state != nullptr);
...
void ChangeDiscreteComPos(const enums::DiscreteComPos new_com_pattern,
                          LegStateBit* leg_state) {
  // leg_state は nullptrではない.
  assert(leg_state != nullptr);

  const LegStateBit state = static_cast<unsigned int>(new_com_pattern)
                            << kShiftToComNum;
