...
SystemMainResultViewer::SystemMainResultViewer(
    ...
    const std::shared_ptr<const ApplicationSettingRecord>&,
...
void SystemMainResultViewer::Main() {
  ...
  while (true) {
    ...
    if (!file_tree.SelectFile(ResultFileConst::kDirectoryPath, -1, "csv",
      ...
      cmdio::Print("No data were found. Terminate.", kError);
    ...
    if (result_importer_.ImportNodeListAndMapState(res_path, &graph,
      ...
      cmdio::Print("Failed to read the file. Exit.", kError);
    }
...
void SystemMainResultViewer::OutputErrorLegPos(
  ...
  cmdio::Print("Outputs data of abnormal values.", OutputDetail::kSystem);
  ...
  cmdio::Print("Output Directory: " + output_path, OutputDetail::kSystem);
