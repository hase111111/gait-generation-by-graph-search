...
struct Quaternion final {
  ...
  constexpr Quaternion(const float w_, const float x_, const float y_,
                       const float z_)
      : w(w_), v(x_, y_, z_) {}
  ...
  //! @return ノルム.
  [[nodiscard]] inline float GetNorm() const noexcept {
  ...
  //! @n q^-1 = q* / |q|^2
  //! @return 逆数.
  [[nodiscard]] inline Quaternion GetInverse() const {
  ...
  //! @return 距離の2乗.
  [[nodiscard]] constexpr float GetDistanceSquared(
      const Quaternion& q) const noexcept {
  ...
  [[nodiscard]] static Quaternion MakeByAngleAxis(float rad_angle,
                                                  const Vector3& axis);
...
//! 計算量を減らすことができる.
//! @return 回転後のベクトル.
[[nodiscard]] constexpr Vector3 RotateVector3(const Vector3& vec,
                                              const Quaternion& q) {
