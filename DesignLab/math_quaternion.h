...
struct Quaternion final
{
	...
	bool operator == (const Quaternion& q) const noexcept { return math_util::IsEqual(w, q.w) && (v == q.v); }
	...
	//! @n ノルムとは，ベクトルの大きさのこと．クォータニオンのノルムは，w^2 + x^2 + y^2 + z^2 の平方根で求められる．
	//! @return ノルム．
	...
	//! しかし，dxlibは左手座標系なので，dxlibで描画するときは，この関数を使って左手座標系に変換する必要がある．
...
{
	const Quaternion p{ 0, vec.x, vec.y, vec.z };	// 回転させるベクトルをスカラーが0のクオータニオンに変換．

	// 正規化されたクォータニオンを使う場合は，共役と逆数が等しいので，計算量を減らすことができる．

	assert(math_util::IsEqual(q.GetNorm(), 1.0f));	// 正規化されたクォータニオンを使う場合は，正規化されたクォータニオンを渡す必要がある．
