...
std::unique_ptr<IMapCreator> MapCreatorSelector::Select(
  ...
  if (record.map_create_mode == enums::MapCreateMode::kFromFile) {
    ...
    const TomlFileImporter<SimulationMapParameter>
        simulation_map_parameter_importer(std::move(validator_ptr));

    const std::string simulation_map_param_file_path =
        TomlDirectoryExporter::kTomlFileDirPath +
        record.simulation_map_param_file_name;
    ...
    map_creator =
        std::make_unique<MapCreatorForSimulation>(simulation_map_parameter);
  } else {
