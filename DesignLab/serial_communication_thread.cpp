...
//! @file serial_communication_thread.cpp
...
void SerialCommunicationThread::Loop() {
  ...
  while (true) {
    Sleep(static_cast<int>(
        1000 * kThreadPeriod));  // 指定した時間だけスリープする.(ミリ秒単位)
...
std::vector<std::string> SerialCommunicationThread::GetReadData(
    const int num) const {
  // スレッドの排他制御を行う.
  boost::mutex::scoped_lock lock(mutex_);
...
bool SerialCommunicationThread::Initialize() {
  ...
  serial_handle_ =
      CreateFile(kComName[kComPortNumber].c_str(), GENERIC_READ | GENERIC_WRITE,
  ...
  PurgeComm(serial_handle_, PURGE_TXABORT | PURGE_RXABORT | PURGE_TXCLEAR |
                                PURGE_RXCLEAR);  // バッファーのクリア.
...
bool SerialCommunicationThread::Write() {
  ...
  if (!WriteFile(serial_handle_, write_data_.c_str(),
                 static_cast<DWORD>(write_data_.size()), &write_size, NULL)) {
    // 書き込みに失敗した場合は終了する.
    return false;
  }
