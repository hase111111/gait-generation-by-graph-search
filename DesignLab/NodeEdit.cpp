...
void node_edit::initNode(SNode& _node, const bool _do_random)
{
	...
	_node.Leg[0] = myvector::SVector(100.0f,	-120.0f,	-COM_Z);
	_node.Leg[1] = myvector::SVector(0.0f,		-130.0f,	-COM_Z);
	_node.Leg[2] = myvector::SVector(-100.0f,	-120.0f,	-COM_Z);
	_node.Leg[3] = myvector::SVector(-100.0f,	120.0f,		-COM_Z);
	_node.Leg[4] = myvector::SVector(0.0f,		130.0f,		-COM_Z);
	_node.Leg[5] = myvector::SVector(100.0f,	120.0f,		-COM_Z);

	//脚付け根を原点とした，脚位置4に該当する座標を初期化する．
	_node.Leg2[0] = myvector::SVector(100.0f,	-120.0f,	-COM_Z);
	_node.Leg2[1] = myvector::SVector(0.0f,		-130.0f,	-COM_Z);
	_node.Leg2[2] = myvector::SVector(-100.0f,	-120.0f,	-COM_Z);
	_node.Leg2[3] = myvector::SVector(-100.0f,	120.0f,		-COM_Z);
	_node.Leg2[4] = myvector::SVector(0.0f,		130.0f,		-COM_Z);
	_node.Leg2[5] = myvector::SVector(100.0f,	120.0f,		-COM_Z);
	...
	_node.global_center_of_mass = myvector::SVector(_ex * _x * cos(_angle), _ex * _y * sin(_angle), COM_Z);


	//ロールピッチヨーで回転を表現する．ロボットの重心を中心にして回転する． 
	_node.rot = myvector::SRotator(0, 0, 0);
