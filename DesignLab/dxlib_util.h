...
//! @brief このプログラムで使用しているVectorと，
//! Dxlibの座標を示す VECTOR を変換する．
//! @n ロボット座標系は右手座標系，
//! Dxlibは左手座標系(工学は右手・ゲームライブラリは左手が多い)
//! なので y を反転する．
//! @param[in] vec 変換前の座標．
//! @return 変換後の座標．
inline Vector3 ConvertDesignLabVec(const VECTOR& vec)
{
...
void DrawCube3DWithTopPos(
    const VECTOR& top_pos, float side_len, unsigned int color);


//! @brief 3D空間に六角形を描画する．
//! @attention 各点は同一平面上にあるものにしないと，描画が歪む．
...
void DrawHexagonalPrism(
    const std::array<VECTOR, 6>& vertex, float height, unsigned int color);
