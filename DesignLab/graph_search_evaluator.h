...
class GraphSearchEvaluator final {
  using Tag =
      GraphSearchEvaluationValue::Tag;  // 読みやすさのため,別名をつける.
 ...
 public:
  ...
  //! @param[in] evaluation_priority_list 優先的に評価を行う Tag
  //! から順に並べたリスト.
  GraphSearchEvaluator(
      const std::map<Tag, EvaluationMethod>& evaluation_method_map,
  ...
  //! @param[in] return_true_case_of_equal 左右の評価値が等しい場合に true
  //! を返すかどうか. デフォルトは true.
 ...
 private:
  const std::map<Tag, EvaluationMethod>
      evaluation_method_map_;  //!< Tag に対する評価方法を格納したマップ.
