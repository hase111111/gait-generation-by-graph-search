...
//! @file dxlib_gui_display_node_switcher.cpp
...
DxlibGuiDisplayNodeSwitcher::DxlibGuiDisplayNodeSwitcher(const int window_x,
  ...
  button_.push_back(std::make_unique<SimpleButton>(
      "再生\n停止", kButtonLeftX + (button_interval + button_width) * 2,
      kButtonTopY, button_width, button_width));
  ...
  button_.push_back(std::make_unique<SimpleButton>(
      ...
      kButtonTopY + (button_interval + button_width), button_width,
      button_width));
  ...
  button_.push_back(std::make_unique<SimpleButton>(
      "Prev Simu", kButtonLeftX + (button_interval + button_width) * 1,
      kButtonTopY + (button_interval + button_width) * 2, button_width * 2,
      button_width));
  button_.back()->SetActivateFunction([this]() { MovePrevSimulation(); });

  button_.push_back(std::make_unique<SimpleButton>(
      "Next Simu", kButtonLeftX + (button_interval + button_width) * 3,
      kButtonTopY + (button_interval + button_width) * 2, button_width * 2,
      button_width));
...
void DxlibGuiDisplayNodeSwitcher::Draw() const {
  ...
  DrawFormatStringToHandle(
      text_left_x, gui_top_pos_y_ + kTitleBarHeight + 10, text_color,
      font_handle_, "[シミュレーション%d回目(全%d回)]", simulation_num_ + 1,
      GetAllSimulationNum(), display_node_num_, all_node_num_);
  ...
  } else if (simulation_num_ ==
             static_cast<int>(simulation_end_index_.size())) {
    start_node_num =
        static_cast<int>(simulation_end_index_[simulation_num_ - 1] + 1);
    end_node_num = static_cast<int>(all_node_num_ - 1);
  } else {
    start_node_num =
        static_cast<int>(simulation_end_index_[simulation_num_ - 1] + 1);
  ...
  DrawFormatStringToHandle(text_left_x, gui_top_pos_y_ + kTitleBarHeight + 30,
                           text_color, font_handle_, "表示ノード : %d (%d～%d)",
                           display_node_num_, start_node_num, end_node_num,
                           all_node_num_ - 1);

  DrawFormatStringToHandle(text_left_x, gui_top_pos_y_ + kTitleBarHeight + 50,
                           text_color, font_handle_, "全ノード : %d ",
                           all_node_num_ - 1);

  DrawFormatStringToHandle(text_left_x, gui_top_pos_y_ + kTitleBarHeight + 70,
  ...
  DrawFormatStringToHandle(text_left_x, gui_top_pos_y_ + kTitleBarHeight + 150,
                           text_color, font_handle_,
                           "アニメーションの\n     速度変更");
}
