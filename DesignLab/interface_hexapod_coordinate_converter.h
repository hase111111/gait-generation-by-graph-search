...
//! @brief
//! ロボットは重心位置・アクチュエータなど様々な点を基準とする座標系を持つ.
...
//!
//! 運動学の教科書とか読む感じ,
//! この座標系の取り方とは違うのだが,先行研究で使われた手法であり,
//! 実用上問題ないのでこのまま.
class IHexapodCoordinateConverter {
 public:
  ...
  //! @param[in] consider_rot ロボットの姿勢を考慮するかどうか.
  //! falseなら回転を考慮しない.
  //! @return 脚座標系の座標.脚先座標系とは脚の付け根を原点とし,
  //! 軸はロボット座標系と同様な座標系.
  virtual Vector3 ConvertGlobalToLegCoordinate(
  ...
  //! falseなら回転を考慮しない.
  //! @return グローバル座標系の座標.
  virtual Vector3 ConvertLegToGlobalCoordinate(
      const Vector3& converted_position, int leg_index,
      const Vector3& center_of_mass_global, const Quaternion& robot_quat,
      bool consider_rot) const = 0;
  ...
  //! falseなら回転を考慮しない.
  //! @return グローバル座標系の座標.
  virtual Vector3 ConvertRobotToGlobalCoordinate(
