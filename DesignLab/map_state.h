...
public:
	...
	inline designlab::Vector3 GetMapPoint(const size_t num) const
	{
		assert(num < map_point_.size());
	...
	//! @param [in] num 何番目の脚設置可能点を変更するか．
	//! @param [in] point 脚設置可能点の座標．
	inline void SetMapPoint(const size_t num, const designlab::Vector3& point)
	{
	...
	//! @brief 脚設置可能点の座標を設定する
	//! @param [in] point 脚設置可能点の座標．
	inline void SetMapPoint(const std::vector<designlab::Vector3>& point)
	{
		map_point_ = point;
	}
