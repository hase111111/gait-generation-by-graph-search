...
class MapState final {
 public:
  ...
  //! @brief 脚設置可能点の総数を返す．
  //! @return 脚設置可能点の総数
  inline size_t GetMapPointSize() const noexcept { return map_point_.size(); }
  ...
  inline void ClearMapPoint() noexcept { map_point_.clear(); }
 ...
 private:
  std::vector<Vector3>
      map_point_;  //!< ロボットが歩くマップ．脚設置可能点の集合で表現される．

  static_assert(kMapPointDistance > 0.0f,
