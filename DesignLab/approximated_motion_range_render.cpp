...
//! @file approximated_motion_range_render.cpp
...
ApproximatedMotionRangeRender::ApproximatedMotionRangeRender(
    ...
    : validator_(validator), converter_(converter) {
  InitPosState();
}
    ...
    for (int j = 0; j < kDivNum; ++j) {
      for (int k = 220; k < kDivNum; ++k) {
        if (pos_state_[i][j][k] == PosState::kInRangeDrawable) {
          const Vector3 pos =
              Vector3(center_.x + (i - kDivNum / 2) * EdgeLength / kDivNum,
                      center_.y + (j - kDivNum / 2) * EdgeLength / kDivNum,
                      center_.z + (k - kDivNum / 2) * EdgeLength / kDivNum);

          const Vector3 global_pos = converter_->ConvertLegToGlobalCoordinate(
              pos, kLegIndex, node_.center_of_mass_global_coord, node_.posture,
              true);

          dxlib_util::DrawCube3D(dxlib_util::ConvertToDxlibVec(global_pos),
                                 EdgeLength / kDivNum, GetColor(255, 0, 0));
        }
      }
    }
  }
...
void ApproximatedMotionRangeRender::InitPosState() {
  ...
  for (int i = 0; i < kDivNum; ++i) {
    for (int j = 0; j < kDivNum; ++j) {
      for (int k = 0; k < kDivNum; ++k) {
        // 自身と隣接する 8 + 9 + 9 = 26
        // 点を調べ,範囲内が5点以下なら描画可能とする.
        if (pos_state_[i][j][k] == PosState::kNotInRange) {
          continue;
        }
        ...
        for (int di = -1; di <= 1; ++di) {
          for (int dj = -1; dj <= 1; ++dj) {
            for (int dk = -1; dk <= 1; ++dk) {
              if (i + di < 0 || i + di >= kDivNum || j + dj < 0 ||
                  j + dj >= kDivNum || k + dk < 0 || k + dk >= kDivNum) {
                continue;
              }

              if (pos_state_[i + di][j + dj][k + dk] == PosState::kInRange ||
                  pos_state_[i + di][j + dj][k + dk] ==
                      PosState::kInRangeDrawable) {
                ++in_range_num;
              }
            }
          }
