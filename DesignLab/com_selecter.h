...
public:
	...
	bool getComFromPolygon(const dl_vec::SPolygon2& polygon, const ComType::EComPattern _com_pattren, dl_vec::SVector& _output_com) const;

private:

	//! @brief 候補地点を生成する
	void makeComCandidatePoint(const dl_vec::SPolygon2& polygon, std::vector<dl_vec::SVector2>& _output_coms) const;

	//! @brief 絶対安全余裕を計算し，マージンを外れていないか調べる
	bool isInMargin(const dl_vec::SPolygon2& polygon, const dl_vec::SVector2& _com) const;
