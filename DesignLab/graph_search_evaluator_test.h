...
TEST_SUITE("GraphSearchEvaluator") {
  ...
  TEST_CASE("LeftIsBetterTest_WhenLeftIsBetter_ShouldReturnTrue") {
    SUBCASE("LowerValueIsBetter") {
      ...
      GraphSearchEvaluator evaluator(
          {{kFirstTag, evaluation_method}, {kSecondTag, evaluation_method}},
  ...
  TEST_CASE("LeftIsBetterTest_WhenValuesAreEqual_ShouldReturnTrueOrFalse") {
    ...
    const GraphSearchEvaluator evaluator(
        {{kFirstTag, evaluation_method}, {kSecondTag, evaluation_method}},
        {kFirstTag, kSecondTag});
  ...
  TEST_CASE("InitializeEvaluationValueTest_ShouldReturnMinEvaluationValue") {
    ...
    GraphSearchEvaluator evaluator({{kFirstTag, evaluation_method_min},
                                    {kSecondTag, evaluation_method_max}},
                                   {kFirstTag, kSecondTag});

    const GraphSearchEvaluationValue actual =
        evaluator.InitializeEvaluationValue();

    const GraphSearchEvaluationValue expected = {
        .value = {{kFirstTag,
                   GraphSearchEvaluationValue::
                       kMaxEvaluationValue},  // 最低評価となるのは,最も高い値.
                  {kSecondTag,
                   GraphSearchEvaluationValue::
                       kMinEvaluationValue}},  // 最低評価となるのは,最も低い値.
    };
