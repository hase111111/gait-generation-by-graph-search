...
class IHexapodJointCalculator {
 public:
  ...
  //! @return 全ての関節の状態.
  [[nodiscard]] std::array<HexapodJointState, HexapodConst::kLegNum>
  CalculateAllJointState(const RobotStateNode& node) const noexcept {
  ...
  //! @return 指定した脚の関節の状態.
  [[nodiscard]] virtual HexapodJointState CalculateJointState(
  ...
  [[nodiscard]] bool IsValidAllJointState(
  ...
  //! @param[in] joint_state 関節の状態.
  //! @return 計算が正しくできているならば true を返す.
  [[nodiscard]] virtual bool IsValidJointState(
