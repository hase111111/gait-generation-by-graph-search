...
void DrawCube3D(const VECTOR& center_pos, const float side_len,
...
void DrawCube3DWithTopPos(const VECTOR& top_pos, const float side_len,
                          const unsigned int color) {
  DrawCube3D(VSub(top_pos, VGet(0, 0, side_len / 2)), side_len, color);
}

void DrawHexagon(const std::array<VECTOR, 6>& vertex,
                 const unsigned int color) {
  // 3D描画の関数は3角形を基本単位とするので,
...
void DrawHexagonalPrism(const std::array<VECTOR, 6>& vertex, const float height,
  ...
  const VECTOR center_to_top = VScale(
      VNorm(VCross(VSub(vertex[0], vertex[1]), VSub(vertex[0], vertex[2]))),
      height / 2.0f);

  // 上面の頂点.
  const std::array<VECTOR, 6> vertex_top = {
      VAdd(vertex[0], center_to_top), VAdd(vertex[1], center_to_top),
      VAdd(vertex[2], center_to_top), VAdd(vertex[3], center_to_top),
      VAdd(vertex[4], center_to_top), VAdd(vertex[5], center_to_top)};

  // 底面の頂点.
  const std::array<VECTOR, 6> vertex_bottom = {
      VSub(vertex[0], center_to_top), VSub(vertex[1], center_to_top),
      VSub(vertex[2], center_to_top), VSub(vertex[3], center_to_top),
      VSub(vertex[4], center_to_top), VSub(vertex[5], center_to_top)};
  ...
  for (int i = 0; i < 6; i++) {
    DrawTriangle3D(vertex_top[i % 6], vertex_top[(i + 1) % 6],
                   vertex_bottom[i % 6], color, TRUE);

    DrawTriangle3D(vertex_top[(i + 1) % 6], vertex_bottom[i % 6],
                   vertex_bottom[(i + 1) % 6], color, TRUE);
  }
}
