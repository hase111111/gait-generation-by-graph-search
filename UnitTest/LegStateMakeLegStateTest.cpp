...
namespace LegStateEditTest
{
	...
	TEST(MakeLegStateFunc, MakeLegState_ChangeGround)
	{
		...
		_expected = 0b110011001100110001001100 | ComType::convertComPatternToBit(_com_pattern) << LegStateEdit::SHIFT_TO_COM_NUM;
		EXPECT_EQ(LegStateEdit::makeLegState(_com_pattern, _ground, _leg_pos), _expected);

		_ground[0] = true; _ground[1] = true; _ground[2] = false;
		_ground[3] = true; _ground[4] = true; _ground[5] = true;
		_expected = 0b110011001100010011001100 | ComType::convertComPatternToBit(_com_pattern) << LegStateEdit::SHIFT_TO_COM_NUM;
		EXPECT_EQ(LegStateEdit::makeLegState(_com_pattern, _ground, _leg_pos), _expected);

		//全脚遊脚のデータを作成する．
		_ground[0] = false; _ground[1] = false; _ground[2] = false;
		_ground[3] = false; _ground[4] = false; _ground[5] = false;
		_expected = 0b010001000100010001000100 | ComType::convertComPatternToBit(_com_pattern) << LegStateEdit::SHIFT_TO_COM_NUM;
		EXPECT_EQ(LegStateEdit::makeLegState(_com_pattern, _ground, _leg_pos), _expected);
	}
	...
	TEST(MakeLegStateFunc, MAkeLegState_ChangeLegPos)
	{
		...
		_expected = 0b101110111011101110111011 | ComType::convertComPatternToBit(_com_pattern) << LegStateEdit::SHIFT_TO_COM_NUM;
		EXPECT_EQ(LegStateEdit::makeLegState(_com_pattern, _ground, _leg_pos), _expected);

		_leg_pos[0] = 2; _leg_pos[1] = 2; _leg_pos[2] = 2;
		_leg_pos[3] = 2; _leg_pos[4] = 2; _leg_pos[5] = 2;
		_expected = 0b101010101010101010101010 | ComType::convertComPatternToBit(_com_pattern) << LegStateEdit::SHIFT_TO_COM_NUM;
		EXPECT_EQ(LegStateEdit::makeLegState(_com_pattern, _ground, _leg_pos), _expected);
